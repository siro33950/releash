 ...
 * 提示してよい session を返す。workflow step として起動された session は
 ...
 *
 * @returns active session が存在し、かつ workflow step session でない場合はその session。
...
export function useDisplayedActiveSession(): ChatSession | null {
	...
	return useMemo(() => {
		...
		if (activeSession.workflowStepSession) return null;
