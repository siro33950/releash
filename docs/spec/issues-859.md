...
- 編集は外部エディタに委譲し、アプリ内にYAMLエディタは持たない
...
- 「外部エディタで編集」ボタンでYAMLファイルが外部エディタで開かれる
...
Feature: ワークフロースキーマ & ストレージ
  ...
  Rule: 各ワークフローに外部エディタで編集するボタンが表示される
    Scenario: ワークフローを外部エディタで編集する
      Given ワークフロー一覧が表示されている
      When ワークフローの「外部エディタで編集」ボタンを押す
      Then 該当のYAMLファイルが外部エディタで開かれる
```
