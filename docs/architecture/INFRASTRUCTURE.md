...
- **変換しない。** 外部世界の都合を内側の言語へ写すのは gateway、内側の結果を転送の形へ写すのは presenter の責務である（[GATEWAY.md](./GATEWAY.md)、[PRESENTER.md](./PRESENTER.md)）。変換を始めたら、それはもう infrastructure ではない
...
- **横断的関心事の包みを定義しない。** 包みの定義は common に置き、外部世界への呼び出しに掛ける

## gateway との境界

判定はひとつ。**変換しているか。**

| 対象 | infrastructure（そのまま扱う） | gateway / presenter（変換する） |
|---|---|---|
...
外部世界に触る処理が内側の語彙を必要とし始めたら、その部分は gateway か presenter へ切り出す。infrastructure に残すのは、ドメインを知らなくても書ける部分だけである。
