...
- **フロント（クライアント）の都合をドメインに漏らしてはならない。** 表示・転送のためにその形が必要な型は DTO（read model）であり、`usecase`（DTO）や `adaptor/gateway`（read model）側に置く。
...
→ Query 側のモデル構築は [GATEWAY.md](./GATEWAY.md) の read model、[USECASE.md](./USECASE.md) の QueryService を参照。
...
永続化モデル・転送メッセージは、保存・転送の形式を表し、判断を持たない限り、ドメインの規則の複製には当たらない。これらに受理判定や遷移を書いた時点で、それは二つ目の表現になる。DTO は QueryService が返す Response に限定した呼称であり、永続化モデル・転送メッセージとは区別する（[USECASE.md の DTO](./USECASE.md#dto)）。
...
**domain 層に置く port は、ドメインの言語だけで書く。** 引数・戻り値・エラーに外部世界の語彙（`git2` の型、SQL の行、wire の形式、JSON の汎用値、外部システム固有のコード）を出さない。
...
データアクセスの抽象化。戻り値はドメイン固有のエラー型を使う。エラー型は失敗の分類を domain の語彙で持ち、transport の語彙（gRPC のステータスコード、HTTP status 等）を持ち込まない。

### Gateway trait

Repository とは別に、**外部システムとの非永続な対話**を扱う。イベント駆動（監視、ストリーム）は `Stream` を返す形式で定義する。
