...
- 参加者の種類と Agent の同一性はレビュー議論上で区別される: Actor は human と agent を区別し、Agent の同一性は Releash が Agent session から解決した backend / model に基づく。session id は監査情報であり、参加者の同一性判定（List filter / unread）には使わない。
- Resolve は open な Thread に対し参加者種別に関わらず実行できる: Rust の review comment domain は open / resolved の状態判定のみを Resolve の前提とし、participant identity による権限判定は行わない。
- Agent と人間は同じレビュー議論を観測できる: CLI / Tauri command / Remote WebSocket は同じ Rust usecase に接続し、Thread 一覧、詳細、解決情報、新着発見に必要な更新情報を同じ意味で返す。
- 拒否された操作では理由を確認できる: 権限不足、resolved 後 mutation、無効な Thread 作成などの拒否は、入口ごとに独自解釈せず Rust 側の同じ理由分類として返す。
- 既存のレビューコメント導線は新しい議論モデルを表示し操作できる: Desktop UI と Remote UI は新しい Thread / Comment / Resolve の投影 DTO を表示し、人間操作の入力入口として振る舞う。意味解釈と権限判断は持たない。
- Thread の履歴は監査目的で確認できる: 永続化される履歴は Thread の作成、Comment 追加、Resolve を時系列に確認できる契約を持つ。通常の List / Get は現在状態の投影を返し、監査用の取得では履歴を返す。
- 並行した操作は確定した順序に従って一貫した状態になる: 書き込み境界は worktree ごとに順序を確定し、Comment は欠落させず、Resolve は最初に有効確定したものだけを終状態にする。
...
- Review comment は Thread / Comment / Resolve の議論モデルとして扱う。既存の単一コメントモデルでは、複数参加者の議論、解決権限、監査履歴を一貫して表せないため。Thread に対する参加者ごとの「立場」表明 (agree / disagree 等) は本機能の対象としない。意見の相違は Comment 本文で表現する。
- Source of truth は worktree ごとの append-only 履歴とし、現在状態は Rust 側で投影する。Comment の append-only と Resolve の終状態を同じ順序モデルで扱えるため。
- Resolve は participant identity に依存しない単純な状態遷移操作とする。誰が起票・解決したかを問わず、open であれば resolved に遷移できる。多数決や合意フローを必要としないことで、Agent / 人間どちらの参加経路からでも同じ手順で議論を閉じられる。
...
## Contracts
- CLI contract: Agent は CLI から review Thread の Create / List / Get / Resolve、Comment Append、History 取得を実行できる。mutation は成功時に確定済みの結果を返し、拒否時には理由を機械的に判別できるエラーを返す。CLI は `--session-id` を唯一の参加者コンテキスト境界とし、worktree は session から解決する (`--worktree` 等の worktree 指定引数は提供しない)。
- Tauri command contract: Desktop UI は人間操作として Thread / Comment / Resolve / History を扱える command を呼び出す。返却値は UI がそのまま表示判断に使える投影 DTO とする。
- Remote message contract: Remote client は WebSocket 経由で Desktop UI と同等の human review 操作を要求できる。応答は同じ Thread / Comment / Resolve の意味を持つ。
- List contract: Thread 一覧は worktree を必須境界とし、以下 5 軸で絞り込める (各軸は AND、未指定は絞らない):
  - `author`: `self` (自分が作成した Thread) / `other` (自分以外が作成した Thread)
  - `state`: `open` / `resolved`
  - `unread`: 自分の最後の Comment 追記時刻以降に他者の Comment 追記があるか (true / false)。Resolve は「Comment 追記」に含めない
...
- Get contract: Thread 詳細は Comment の時系列、Thread 状態、Resolve 情報、更新検出に必要な情報を返す。Agent の変化発見は push 通知ではなく List / Get の再取得で成立する。
...
- Agent mutation flow: Agent CLI invocation は Releash が管理する session context から Agent Actor を確定し、review usecase に要求を渡す。usecase は domain rule を適用し、gateway が worktree 履歴へ確定し、CLI は確定後の Thread / Comment / Resolve 結果を返す。
...
## State Ownership
- Thread / Comment / Resolve の正: Rust persistence gateway が管理する worktree ごとの review comment 履歴。
- 現在状態の投影: Rust review usecase / gateway。UI や Remote は独自に Resolve 可否や状態遷移を再計算しない。
- Domain invariants: Rust review comment domain。append-only、resolved 終状態をここが owner として扱う。
...
## Boundaries
- フロントエンドにレビュー議論の意味論を置かない。権限判定、resolved 後拒否、filter の意味解釈は Rust が担う。
...
- Thread に対する参加者ごとの「立場」(agree / disagree 等) を表現する domain・契約・UI は提供しない。意見の相違は Comment 本文で表現する。
...
- List / Get の filter セットと並び順の詳細は、worktree、file、状態、著者、新着発見に必要な条件を満たす範囲で実装に委ねる。
...
- 同時書き込み時の順序確定方式とファイル保護方式は、Comment 欠落なし、Resolve 単一確定、worktree ごとの破損防止を満たす範囲で実装に委ねる。
