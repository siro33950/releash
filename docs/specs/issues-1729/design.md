...
- 保存済みの実行が旧記法（`output` を含む）の定義を抱えている場合、field 削除後にそのNode定義を現行の型へ変換できなくなる。旧記法の互換維持と自動移行は行わない。保存記録の参照と実行継続は #1744 の方針に従い、Node単位で定義の解釈可否を扱う。Sessionの接続情報・Commandの保存内容を保持し、解釈できない定義や再導出できない成果に依存する処理だけを制限する。
