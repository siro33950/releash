...
Issue、Story、または自由文Requestから、`requirements.md`、`behavior.md`、`design.md`を作成する。3文書を上流から順に作成し、その後3文書を1単位として検証・検討・修正を一度だけ行い、最後に人間が完成文書をレビューする。
...
4. Repairが確定Findingをowner文書へ反映する。上流文書を変更した場合は、下流文書を同時に追随させる。修正後はFinalReviewへ進む。

検証・検討・修正は一度だけ行い、機械同士で収束を繰り返さない。修正の確認と残る問題の裁定はFinalReviewの人間が行う。
...
- 確認は実行中のSessionでそのまま行う。具体的な質問を提示し、人間の回答を待つ。回答を得るまで完了を提出しない。文書全体への感想やレビューを求めない。
- 通常の文書レビューは、自動処理が完了した後のFinalReviewだけで行う。

## 変更範囲

- 書き込める成果物は、検証済み`spec_dir`内の`requirements.md`、`behavior.md`、`design.md`だけである。
- コード、設定、テスト、参照文書、Knowledgeを変更しない。
- 文書本文をArtifactへ複製しない。Artifactには入力参照、Finding、判断、要約だけを記録する。
