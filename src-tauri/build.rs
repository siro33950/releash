...
fn generate_client_protocol() {
    ...
    for method in &service.method {
        ...
        handlers.push_str(&format!("async fn {name}<'a>(&'a self, _ctx: connectrpc::RequestContext, request: connectrpc::ServiceRequest<'_, rpc::{input}>) -> connectrpc::ServiceResult<impl connectrpc::Encodable<rpc::{output}> + Send + use<'a>> {{ let args = to_wire(&request.to_owned_message())?; let result = self.execute(wire::command_request::Command::{variant}(args)).await?; let headers = response_headers(&result); match result {{ wire::command_result::Command::{variant}(result) => {{ let mut response = connectrpc::Response::new(to_rpc::<rpc::{output}>(&result)?); response.headers = headers; Ok(response) }}, _ => Err(crate::adaptor::protocol::connect::classified_error(crate::other::AppError::new(\"Mismatched command result\"))) }} }}\n"));
