...
pub(super) fn handle_delete_comment(req: &DeleteComment, state: &WsServerState) -> Option<WsMessage> {
    if let Some(app) = &state.app_handle {
...
pub(super) fn handle_update_comment(req: &UpdateComment, state: &WsServerState) -> Option<WsMessage> {
    if let Some(app) = &state.app_handle {
