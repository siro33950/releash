...
impl FanoutSpec {
    pub fn artifact_key(&self, slot: FanoutSlot) -> Option<String> {
        let child = self.children.get(slot.child_index)?;
        if self.items.is_some() {
            slot.item_index?
                .checked_mul(self.children.len())?
                .checked_add(slot.child_index)
    ...
    pub fn child_entry_for_artifact_key(&self, key: &str) -> Option<&ChildEntry> {
        if self.items.is_some() {
            let index = key.parse::<usize>().ok()?;
            if index.to_string() != key {
                return None;
            }
            self.children.get(index.checked_rem(self.children.len())?)
        } else {
