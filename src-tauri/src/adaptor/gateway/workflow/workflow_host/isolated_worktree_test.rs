...
use crate::domain::workflow::{
    IsolatedWorktree, IsolatedWorktreeGateway, NodeFact, ProcessExitedFact,
};
use std::sync::atomic::{AtomicBool, AtomicUsize, Ordering};
use std::sync::Mutex as StdMutex;

#[derive(Default)]
struct TestWorktrees {
    calls: StdMutex<Vec<(String, IsolatedWorktree)>>,
...
#[derive(Default)]
struct TestSessions {
    prepared: StdMutex<Vec<(String, String, String)>>,
    activated: StdMutex<Vec<String>>,
    recovered: StdMutex<Vec<String>>,
    dispatched: StdMutex<Vec<String>>,
    dispatch_fails_on: StdMutex<Option<String>>,
    recovery_fails: AtomicBool,
    preparation_fails: AtomicBool,
}
...
impl Fixture {
    ...
    async fn start(&self, nodes: &str) -> String {
        self.start_at(nodes, "/repo-worktrees/development").await
    }

    async fn start_at(&self, nodes: &str, root: &str) -> String {
    ...
    fn with_repository() -> (Self, String) {
    ...
    async fn persist_started(&self, nodes: &str, root: &str) -> RuntimeCommitSnapshot {
    ...
    fn restarted_host(&self) -> WorkflowRuntimeHost {
    ...
    async fn command_artifact(&self, execution_id: &str) -> serde_json::Value {
...
async fn test_隔離合成子_子開始commitの状態通知は一度だけ送る() {
    ...
    let leaves = fixture
        ...
        .prepare_isolated_starts(
            ...
            "/repo",
            vec![NodeStart::PrepareComposite(starts)],
        )
        .await
        .unwrap();
    // Then
