//! [05] Command / Event Boundary: workflow driver 内部の node 完了 / 失敗遷移の
//! typed 表現。
//!
//! 外部 mutation は usecase / gateway が resolved runtime primitive wrapper を直接呼ぶ。
//! 本 enum は driver 内部の event 発行点を typed に保つための internal-only 境界であり、
...
pub(crate) enum InternalNodeCommand {
    /// [05] internal-only: driver 内部の node 完了遷移。発行 event は
    ...
    /// [05] internal-only: driver 内部の node 失敗遷移。発行 event は
