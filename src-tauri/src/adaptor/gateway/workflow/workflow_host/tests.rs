...
async fn insert_ready_agent_process_for_internal_turn_test(
    ...
    agent_runtime
        ...
        .await
        .expect("test backend session must start");
}
...
#[test]
fn node_session_tab_cleanup_is_view_only_and_preserves_history() {
    let tmp = tempfile::TempDir::new().unwrap();
    let store = crate::test_support::build_session_store();
...
fn validate_start_empty_nodes_returns_err() {
    ...
    let result = DomainWorkflowExecution::validate_start(&workflow, None);
...
fn validate_start_active_workflow_returns_err() {
    ...
    let result = DomainWorkflowExecution::validate_start(&workflow, Some(&existing));
    assert!(result.is_err());
...
fn validate_start_completed_workflow_allows_restart() {
    ...
    existing.force_state_for_test(RuntimeExecutionState::Completed);
    let result = DomainWorkflowExecution::validate_start(&workflow, Some(&existing));
    assert!(result.is_ok());
}
...
fn is_terminal_aborted() {
    let mut exec = make_exec(0);
    exec.force_state_for_test(RuntimeExecutionState::Aborted);
    assert!(exec.is_terminal());
}
...
fn approved_policy_masks_raw_secrets_before_state_variables_history_and_injection() {
    ...
    let outcome = WorkflowRuntimeHost::apply_approval_application(
        &mut exec,
        ApprovalApplication {
...
fn insert_single_node_execution(
    execs: &mut HashMap<String, DomainWorkflowExecution>,
    node: NodeDefinition,
) {
...
async fn start_node_session_with_deps_skips_side_effects_when_prompt_synthesis_fails() {
    ...
    //   (d) `driver.session_workflow_refs` が空のままであること
    ...
    let deps = RecordingNodeSessionDeps::default();
    let result = driver.start_node_session_with_deps(&deps, "/repo").await;
...
async fn test_workflow_session_node_初期指示を一度だけ起動時promptへ渡す() {
    ...
    seed_single_node_facet_contents_for_test(&driver, "instruction-node", facet_contents).await;
...
fn latest_assistant_output_after_approval_chat_adjustment_is_selected() {
    ...
    let output = WorkflowRuntimeHost::extract_last_assistant_text_from_session(&session).unwrap();
...
fn apply_approval_application_records_approved_policy_and_advances_once() {
    ...
    let first = WorkflowRuntimeHost::apply_approval_application(
        &mut exec,
        ApprovalApplication {
            effective_result: "approved".to_string(),
    ...
    let duplicate = WorkflowRuntimeHost::apply_approval_application(
        &mut exec,
        ApprovalApplication {
            effective_result: "approved".to_string(),
            artifact: Some(serde_json::json!({
...
fn make_on_exhausted_workflow() -> WorkflowDefinition {
    WorkflowDefinition {
...
fn make_on_exhausted_depth_exceeded_workflow() -> WorkflowDefinition {
    WorkflowDefinition {
...
/// finding 13 対応: `return 値 execution_id = DomainWorkflowExecution.id = active summary の execution_id
/// = workflow_executions/{execution_id}.json の execution_id` の一致を driver レベルで検証する。
#[tokio::test]
async fn engine_execution_id_consistency_across_execution_and_execution_store_metadata() {
    ...
    driver.executions.lock().await.insert(exec.id.clone(), exec);
    driver
        .execution_store
    ...
    let active = driver.list_active_executions().await;
    ...
    assert_eq!(
        driver.execution_id_for_worktree(worktree_path).await,
        Some(execution_id.clone())
    );
    assert_eq!(
        driver.resolve_worktree_by_execution(&execution_id).await,
...
async fn engine_validate_start_rejects_duplicate_active_execution_on_same_worktree() {
    ...
    let result = DomainWorkflowExecution::validate_start(&workflow, existing);
    match result {
...
/// driver が状態遷移を反映した際に Execution Store の active / completed 一覧および
...
async fn engine_state_transitions_sync_to_execution_store_active_and_completed() {
    ...
    .await
    .unwrap();
    let active_after = driver.list_active_executions().await;
    assert!(
...
fn make_minimal_workflow() -> WorkflowDefinition {
    WorkflowDefinition {
        name: "driver-test-wf".to_string(),
...
fn validate_workflow_shape_rejects_empty_and_accepts_command_workflows_without_side_effects() {
    ...
    let command = WorkflowDefinition {
        name: "wf".to_string(),
        description: "".to_string(),
        builtin: false,
        schemas: Default::default(),
...
async fn execution_store_sync_failure_rolls_engine_projection_back_to_active_state() {
    ...
    driver
        ...
        .await
        .unwrap();
    driver.executions.lock().await.insert(
        execution_id.clone(),
        make_exec_with(
            &execution_id,
            worktree_path,
...
/// 取り違えないことを driver state 観測で保証する。
#[tokio::test]
async fn abort_workflow_by_execution_id_does_not_modify_sibling_active_execution_state() {
    ...
    assert!(matches!(
        driver.abort_target_lookup(&terminal_execution_id).await,
...
/// execution_id と、`DomainWorkflowExecution.id` / active summary / workflow_executions/{execution_id}.json が
...
async fn start_workflow_core_records_execution_id_and_rejects_duplicate_worktree() {
    ...
    let empty_request_execution_id = driver
        .start_workflow_common_core_for_test(
            make_minimal_workflow(),
...
async fn start_workflow_duplicate_reservation_does_not_leak_metadata_or_refs() {
    ...
    assert_eq!(
        active_only_summary(&driver).await,
...
async fn active_only_summary(driver: &WorkflowRuntimeHost) -> Vec<String> {
    driver
...
mod dispatch_boundary_tests {
    ...
    /// builtin workflow が使う `claude-opus-5` / `gpt-5.6-sol` を production と同一経路で
    ...
    fn make_approval_only_workflow() -> WorkflowDefinition {
        WorkflowDefinition {
    ...
    fn make_running_session_workflow() -> WorkflowDefinition {
        WorkflowDefinition {
            name: "boundary-wf".to_string(),
            description: "test".to_string(),
            builtin: false,
    ...
    fn install_test_fanout(exec: &mut DomainWorkflowExecution, children: Vec<FanoutChildRuntime>) {
        let execution_id = exec.id.clone();
        let started_at = exec.started_at;
        let (parent_node, parent_attempt, parent_node_execution_id) = {
            let parent = exec
                .node_executions
                .first_mut()
                .expect("fanout parent NodeExecution");
            let _ = parent.replay_started();
            parent.kind = NodeKindName::Fanout;
            (parent.node_name.clone(), parent.attempt, parent.id.clone())
        };
    ...
    ) -> crate::adaptor::gateway::workflow::workflow_host::execution_state::DomainWorkflowExecution
    {
        ...
        let approval_child = exec
            .node_executions
            .iter_mut()
            .find(|execution| execution.id == "ne-review-child-0")
            .unwrap();
        let _ = approval_child.wait_for_approval();
        exec
    }
    ...
    async fn seed_pending_sequential_turn_completion(
        ...
        let workflow = WorkflowDefinition {
            name: "turn-recovery-wf".to_string(),
            description: String::new(),
            builtin: false,
            schemas: Default::default(),
            nodes: vec![
                make_test_node(
                    ...
                    TestKind::Session,
                    "review-acceptance",
                    vec![Rule::Next("verify".to_string())],
                    None,
                ),
                make_test_node(
                    "verify",
        ...
        let node_execution_id = format!("{execution_id}-fix-1");
        store
            .register_active_execution(WorkflowExecutionMetadata {
                execution_id: execution_id.to_string(),
                workflow_name: workflow.name.clone(),
                status: ExecutionStatus::Running,
                worktree_path: worktree_path.to_string(),
                current_node: Some("fix".to_string()),
                created_from: ExecutionOrigin::Agent,
                started_at: 100.0,
                updated_at: 101.0,
                completed_at: None,
        ...
        WorkflowEventLog::new(data_dir)
            .append_batch(&[
                WorkflowEvent::ExecutionStarted {
                    ...
                    worktree_path: worktree_path.to_string(),
                    created_from: ExecutionOrigin::Agent,
                    request: "repair the issue".to_string(),
                    permission_mode: "ask".to_string(),
        ...
        WorkflowTurnCompleteRecoveryCommand {
            notification: WorkflowTurnCompleteNotification {
                ...
                exit_code: 0,
                final_text_parts: vec!["fixed".to_string(), " and verified".to_string()],
                failure_signal: None,
    ...
    async fn restart_retires_legacy_success_without_bypassing_completion_handshake() {
        ...
        // A fresh driver has no runtime-local session map. Wrong durable
        ...
        assert!(driver
            .recover_turn_complete(app.handle(), &session_store, &agent_runtime, mismatched,)
            .await
            .is_err());
        assert_eq!(
    ...
    async fn restart_retires_legacy_fanout_success_without_completing_the_child() {
        ...
        let execution_id = uuid::Uuid::new_v4().to_string();
        let workflow = WorkflowDefinition {
            name: "fanout-turn-recovery-wf".to_string(),
            description: String::new(),
            builtin: false,
            schemas: Default::default(),
            nodes: vec![
                make_fanout_node("review-all", vec!["review-a", "review-b"]),
        ...
        previous_store
            .register_active_execution(WorkflowExecutionMetadata {
                ...
                workflow_name: workflow.name.clone(),
                status: ExecutionStatus::Running,
                worktree_path: "/wt/fanout-turn-recovery".to_string(),
                current_node: Some("review-all".to_string()),
                created_from: ExecutionOrigin::Agent,
                started_at: 100.0,
                updated_at: 102.0,
                completed_at: None,
            ...
            .await
            .unwrap();
        let parent_id = format!("{execution_id}-review-all-1");
        let child_a_id = format!("{execution_id}-review-a-1");
        let child_b_id = format!("{execution_id}-review-b-1");
        let session_a = format!("session-a-{execution_id}");
        let session_b = format!("session-b-{execution_id}");
        WorkflowEventLog::new(&data_dir)
            .append_batch(&[
                WorkflowEvent::ExecutionStarted {
                    execution_id: execution_id.clone(),
                    ...
                    worktree_path: "/wt/fanout-turn-recovery".to_string(),
                    created_from: ExecutionOrigin::Agent,
                    request: "review everything".to_string(),
                    permission_mode: "ask".to_string(),
                    definition: workflow.clone(),
                    timestamp: 100.0,
                },
                WorkflowEvent::NodeStarted {
                    ...
                    node_name: "review-all".to_string(),
                ...
                WorkflowEvent::NodeStarted {
                    execution_id: execution_id.clone(),
                    node_execution_id: child_a_id.clone(),
                    node_name: "review-a".to_string(),
                    kind: NodeKindName::Session,
                    attempt: 1,
                    fanout_parent: Some(FanoutParentRef {
                        parent_node: "review-all".to_string(),
                        parent_attempt: 1,
                        item_index: None,
                        child_index: 0,
                    }),
                ...
                WorkflowEvent::SessionAttached {
                    execution_id: execution_id.clone(),
                    node_execution_id: format!("{execution_id}-review-b-1"),
        ...
        let command = WorkflowTurnCompleteRecoveryCommand {
            notification: WorkflowTurnCompleteNotification {
                chat_session_id: session_a,
                exit_code: 0,
                final_text_parts: vec!["review a complete".to_string()],
                failure_signal: None,
                token_usage: Some(WorkflowTurnTokenUsage {
                    input_tokens: 5,
                    output_tokens: 3,
            ...
            node_name: "review-a".to_string(),
            attempt: 1,
            parent_node_name: Some("review-all".to_string()),
    ...
    async fn abort_workflow_by_execution_id_clears_stall_observations_in_live_and_projection() {
        ...
        exec.force_state_for_test(RuntimeExecutionState::Running);
        let _ = exec.node_executions[0].replay_started();
    ...
    async fn insert_execution_and_register_active(
        ...
        driver
            ...
            .await
            .unwrap();
        driver.executions.lock().await.insert(execution_id, exec);
    }
    ...
    fn append_resumable_two_node_events(
        data_dir: &std::path::Path,
        exec: &DomainWorkflowExecution,
    ) {
        ...
        WorkflowEventLog::new(data_dir)
            .append_batch(&[
                ...
                WorkflowEvent::SessionAttached {
                    ...
                    session_id: exec.node_executions[1]
                        .session_id
                        .clone()
                        .expect("the resumable execute node must own a session"),
                    timestamp: 1003.0,
                },
            ])
            .unwrap();
    ...
    async fn start_command_workflow_for_test(
        ...
        let execution_id =
            start_command_workflow_nowait_for_test(app, driver, workflow, worktree_path).await;
        ...
        for node in &mut workflow.nodes {
            match &mut node.kind {
                ...
                NodeKind::Session(session) => {
                    ...
                    // integration isolates driver behavior from the user's facet inventory by
    ...
    fn install_full_pipeline_cli() -> crate::test_support::EnvVarGuard {
        ...
        let llvm_cov_target_dir = std::env::var_os("CARGO_LLVM_COV_TARGET_DIR");
        let under_llvm_cov =
            std::env::var_os("CARGO_LLVM_COV").is_some() || llvm_cov_target_dir.is_some();
        let target_dir = llvm_cov_target_dir
        ...
        // `cargo llvm-cov` instruments child Cargo invocations through inherited environment
        // variables. This fixture only needs a real production CLI process; instrumenting that
        // nested binary can delay command-node completion long enough to make the async workflow
        // oracle observe a false timeout. Keep the test harness instrumented while building its
        // subprocess fixture with ordinary compiler settings in llvm-cov's original target dir.
        if under_llvm_cov {
            for key in [
                "RUSTFLAGS",
            ...
            .map(|execution| {
                (
                    ...
                    execution
                        .fanout_runtime
                        .as_ref()
                        .map(|fanout| (fanout.parent_node_name.clone(), fanout.children.len())),
                )
            });
        let metadata = driver.execution_store().get_execution(execution_id).await;
        let events = match driver.execution_store().configured_data_dir().await {
            Some(data_dir) => WorkflowEventLog::new(&data_dir)
                .read_log(execution_id)
                .unwrap_or_default(),
            None => Vec::new(),
        };
        panic!(
            "fanout '{parent_node}' did not start {expected_count} children in execution \
             '{execution_id}'; live={live:?}; metadata={metadata:?}; events={events:?}"
        );
    }
    ...
    #[allow(clippy::await_holding_lock)] // Serializes process-global environment variables for this integration-style test.
    async fn canonical_full_pipeline_executes_command_fanout_approval_loop_and_switch_path() {
        ...
        wait_for_top_level_session(&driver, &execution_id, "fix_tests").await;
    ...
    async fn command_runtime_start_returns_before_long_running_command_completes_and_abort_still_works(
        ...
        let execution_id = tokio::time::timeout(
            ...
            start_command_workflow_nowait_for_test(&app, &driver, workflow, worktree.path()),
        )
        .await
    ...
    async fn explicit_pause_kills_the_active_command_process_group_and_records_node_pause() {
        ...
        let execution_id =
            start_command_workflow_nowait_for_test(&app, &driver, workflow, worktree.path()).await;
        wait_for_active_command(&driver, &execution_id).await;
    ...
    async fn app_exit_shutdown_is_graceful_and_kills_process_group_without_crash_fact() {
        ...
        driver.shutdown_all_active_commands().await;
    ...
    async fn command_prepared_append_failure_prevents_process_spawn_and_rolls_back_display() {
        ...
        drop(executions);
        assert!(driver.active_commands.lock().await.is_empty());
    }
    ...
    async fn abort_workflow_kills_active_command_without_completion_artifact() {
        ...
        let runtime_engine = driver.clone();
    ...
    /// Spec [04]: atomic mutation 境界。mutation 直前の `DomainWorkflowExecution` snapshot を
    ...
    async fn approval_snapshot_rollback_restores_workflow_execution_fully() {
        ...
        // mutation を適用
        {
            ...
            let _ = WorkflowRuntimeHost::apply_approval_application(
                exec,
                ApprovalApplication {
    ...
    fn add_historical_guarded_node_completions(execution: &mut DomainWorkflowExecution) {
        let mut historical_fix_executions = Vec::new();
        for attempt in 1..=2 {
    ...
    async fn reused_fanout_children_reset_baseline_matches_replay_routing_and_append_rollback() {
        ...
        append_fanout_child_reset_seed_events(&data_dir, &execution);
        insert_execution_and_register_active(&driver, execution, ExecutionOrigin::DesktopUi).await;
        driver.fanout_resume_checkpoints.lock().await.insert(
            execution_id.clone(),
    ...
    async fn abort_target_lookup_propagates_execution_store_read_failure() {
        let mut driver = WorkflowRuntimeHost::new_for_test();
        driver.execution_store =
    ...
    async fn abort_target_lookup_returns_already_terminal_for_terminal_execution() {
        ...
        for terminal_state in [
            ...
            match driver.abort_target_lookup(&execution_id).await.unwrap() {
    ...
    async fn release_terminal_execution_removes_terminal_entries_only() {
        ...
        for (label, terminal_state) in [
            ...
            exec.force_state_for_test(terminal_state);
            driver
                .executions
                .lock()
                .await
            ...
            driver.release_terminal_execution(&execution_id).await;
        ...
        active.force_state_for_test(RuntimeExecutionState::Running);
        driver
            .executions
            .lock()
            .await
        ...
        assert_eq!(driver.executions_len_for_test().await, 1);
    }
    ...
    /// DomainWorkflowExecution / Execution Store / event log は command 受理前 snapshot に戻る。
    #[tokio::test]
    ...
    /// DomainWorkflowExecution / Execution Store / ChatSession workflow_state は mutation 前へ戻る。
    #[tokio::test]
    ...
    async fn start_execution_primitive_rejects_invalid_name_without_state_change() {
        ...
        let result = driver.resolve_start_execution_workflow("../bad").await;
    ...
    async fn start_execution_primitive_accepts_creates_execution_and_appends_event() {
        ...
        let execution_id = driver
            ...
            .unwrap();
        assert!(
            driver.executions.lock().await.contains_key(&execution_id),
            "StartExecution must register a DomainWorkflowExecution"
        );
        assert!(
    ...
    async fn start_execution_validate_start_failure_releases_execution_facet_contents() {
        ...
        assert!(
            driver.execution_facet_contents.lock().await.is_empty(),
            "validate_start rollback must release execution_facet_contents"
        );
        assert!(
            driver.list_active_executions().await.is_empty(),
    ...
    async fn dispatch_abort_execution_rejects_not_found_and_terminal_without_append() {
        ...
        insert_execution_and_register_active(&driver, terminal, ExecutionOrigin::DesktopUi).await;
        let snapshot_before = {
            let mut executions = driver.executions.lock().await;
            let terminal = executions
                .get_mut(&terminal_execution_id)
                .expect("live terminal fixture");
            terminal.force_state_for_test(RuntimeExecutionState::Completed);
            terminal.clone()
        };
        driver
            .execution_store
        ...
        let released_terminal_result = driver
            .abort_workflow_execution(
                app.handle(),
                &session_store,
                &handles,
    ...
    async fn dispatch_targeted_abort_rejects_missing_stale_and_resolved_targets_without_append() {
        ...
        assert!(driver.list_active_executions().await.is_empty());
        assert!(driver.list_completed_executions().await.is_empty());
        assert!(read_dispatch_events(&app, &missing_execution_id).is_empty());
        ...
        insert_execution_and_register_active(&driver, stale_exec, ExecutionOrigin::DesktopUi).await;
        let stale_active_before = driver.list_active_executions().await;
        ...
        drop(execs);
        let stale_active_after = driver.list_active_executions().await;
        ...
        driver
            ...
            .await
            .unwrap();
        let completed_before = driver.list_completed_executions().await;
        ...
        drop(execs);
        let completed_after = driver.list_completed_executions().await;
    ...
    /// metadata sync failure は、event log と driver state を rollback しない。
    ...
    fn make_submit_output_workflow() -> WorkflowDefinition {
        WorkflowDefinition {
    ...
    async fn submit_output_persists_node_output_and_appends_event_when_contract_satisfied() {
        ...
        let node_output = node_output_for(&driver, &execution_id, "review")
            .await
    ...
    async fn arbitrary_contract_submit_redacts_env_secret_in_state_log_and_api_response() {
        ...
        let state = node_output_for(&driver, &execution_id, "review")
            .await
            .unwrap();
    ...
    /// [08] caller の `--type` と driver の expected contract が一致しない場合は拒否され、
    /// 副作用は発生しない。
    #[tokio::test]
    ...
    async fn submit_output_stores_spec_dir_artifact_without_workflow_variable_side_effects() {
        ...
        let exec = driver.executions.lock().await;
