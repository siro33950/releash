...
Requestが空、必須参照を取得できない、配置先を一意に決められない、または権威ある入力が矛盾する場合は推測しない。解決に必要な一つの具体的質問を`question`へ入れ`NEEDS_HUMAN`にする。

正常時は`READY`、空でない`spec_dir`、確認済みの各配列、空の`question`を提出する。本文やKnowledgeをArtifactへ複製しない。
