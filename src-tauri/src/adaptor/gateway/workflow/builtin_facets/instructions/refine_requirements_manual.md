...
`requirements.md`の`[REQUIREMENTS_AMBIGUITY]` Threadを読み、不足の解消内容を一件ずつユーザーと対話し、明確な合意を得て既存の`requirements.md`を更新する。
...
- ContextまたはRequirementsに明示された根拠から一意に決まる場合も、根拠と反映内容をユーザーへ提示し、明確な合意を得る。
- 既存実装は現在の事実を確認する根拠として使用する。ContextまたはRequirementが既存挙動の維持を求めていない限り、既存実装を期待結果の根拠にしない。
- Contextや既存実装から一意に決まらず、複数の要求として成立する判断が残る場合は、その未決定事項だけをユーザーへ確認する。
- Threadに記録された未決定事項、反映する内容、変更しない内容を一件分だけ提示する。
...
- 質問への回答や検討中の発言を合意とみなさず、文書へ反映することへの明確な合意を得る。
- 合意後、そのThreadに必要な変更だけを反映し、実際の変更内容を提示する。
- ユーザーがそのThreadの完了を確認してから次のThreadへ進む。
...
すべての`[REQUIREMENTS_AMBIGUITY]` Open Threadについて、ユーザーが一件ずつ合意した確定内容を`requirements.md`へ反映し、Threadをresolveした場合だけNodeを完了する。
