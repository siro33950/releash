...
`behavior.md`の`[BEHAVIOR_AMBIGUITY]` Threadを読み、Requirementsと確認済みの事実から不足を解消して、既存の`behavior.md`を更新する。
...
- Requirements、Requirements内のContext、既存のBehavior、関連文書、既存実装の確認結果から一意に確定できる内容だけを`behavior.md`へ反映する。
...
確定できたすべての`[BEHAVIOR_AMBIGUITY]` Threadについて、根拠のある内容を`behavior.md`へ反映し、対応するThreadをresolveしたらNodeを完了する。
