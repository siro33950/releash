...
`behavior.md`は、`requirements.md`の各Requirementを満たしたと外部から判定するための、観測可能な受入条件を記録する文書である。受入条件は外部から観測可能なビジネスルールの表現であり、テスト手順、検証コマンド、具体的な再現値を記載しない。

この文書はRequirementを追加、変更、補完しない。内部の構造や実装方法も決定しない。

## フォーマット

受入条件を`## B-xxx`形式で連続して記載し、末尾に要件IDとBehavior IDの対応表を置く。
