...
impl WorkflowMutationContext {
    ...
    /// `WorkflowEvent::CliMutationRequested` 構築時に所有を `execution_id` /
...
/// `node_name` と `contract` を保持して rejected event の `request` フィールドを
...
impl CommandCommitContext {
    ...
    /// `(request_id, requested_at, node_name, contract)` を返す（5-3 修正）。
