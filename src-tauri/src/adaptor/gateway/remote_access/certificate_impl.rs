...
impl CertificateGateway for TlsCertificateGateway {
...
mod tests {
    ...
    fn ensure_self_signed_cert_creates_files() {
        ...
        let gateway = TlsCertificateGateway;

        let (cert_path, key_path) = gateway.ensure_self_signed_cert(ip, dir.path()).unwrap();
    ...
    #[test]
    fn ensure_self_signed_cert_reuses_existing() {
        let dir = tempfile::TempDir::new().unwrap();
        let ip = IpAddr::V4(Ipv4Addr::new(100, 100, 1, 42));
        let gateway = TlsCertificateGateway;

        let (cert1, _) = gateway.ensure_self_signed_cert(ip, dir.path()).unwrap();
        let content1 = std::fs::read_to_string(&cert1).unwrap();

        let (cert2, _) = gateway.ensure_self_signed_cert(ip, dir.path()).unwrap();
