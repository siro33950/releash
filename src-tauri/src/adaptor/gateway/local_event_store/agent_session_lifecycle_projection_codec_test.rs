...
use crate::domain::local_event::{
    ...
    AgentSessionProviderRecord, ProviderSessionOwnershipProjectionRecord, SessionProjectionRecord,
};
use sha2::{Digest, Sha256};

#[test]
fn test_agent_session_lifecycle_projection_codec_semantic_recordを往復できる() {
    let record = SessionProjectionRecord::AgentSession(AgentSessionProjectionRecord {
        ...
        lifecycle: AgentSessionLifecycleRecord::Paused,
    ...
    let decoded =
        decode_session_projection_record_v1(&raw, "agent-session:agent-session-1").unwrap();
...
{
    let legacy = concat!(
        r#"{"schema":"agent_session_projection_v1","id":"agent-session-1","#,
    ...
    let decoded =
        decode_session_projection_record_v1(legacy, "agent-session:agent-session-1").unwrap();

    let SessionProjectionRecord::AgentSession(projection) = decoded else {
