...
use crate::infrastructure::agent_session::claude::runtime_process::ClaudeRuntimeProcess;
use crate::infrastructure::agent_session::claude::runtime_state::ClaudeRuntimeState;
...
use super::process_config::build_process_config;
use super::wire::{
    control_request_subtype, initialize_request, interrupt_request, message_type,
...
impl ClaudeRuntimeState {
    fn from_spec(spec: &SessionSpec) -> Self {
        Self {
            ...
            permission_mode: spec.permission_mode.as_str().to_string(),
    ...
    fn session_spec_with_system_prompt(&self, system_prompt: Option<String>) -> SessionSpec {
        SessionSpec {
            ...
            cwd: self.cwd.clone(),
            permission_mode: runtime_permission_mode(self),
            plan_mode: self.plan_mode,
            permission_profile_id: None,
            model: ModelId::parse(self.model.clone())
                .expect("Claude runtime model originated from a SessionSpec"),
            system_prompt,
            resume: self
                .backend_session_id
                .clone()
...
) -> crate::domain::agent_session::PermissionMode {
    crate::domain::agent_session::PermissionMode::parse(&state.permission_mode)
        .expect("Claude runtime permission mode originated from a SessionSpec")
}
...
impl AgentSessionRuntime for ClaudeSessionRuntime {
    ...
    async fn start_turn(&self, input: TurnInput) -> Result<(), AgentBackendError> {
        ...
        if let Some(spec) = replace_spec {
            ...
            state.permission_mode = input.permission_mode.as_str().to_string();
    ...
    async fn set_model(&self, model: &ModelId) -> Result<(), AgentBackendError> {
        self.state.lock().await.model = model.as_str().to_string();
        self.current_handle()
            .await
...
async fn spawn_runtime_process(
    ...
    let mut process = ClaudeStdioProcess::spawn(build_process_config(cli_path, spec))
        .await
        .map_err(AgentBackendError::Other)?;
            ...
            Ok(Some(StdoutItem::Json(message))) => {
                ...
                }
                {
                    ...
                    convert_state.wire_mode =
                        claude_wire_mode(runtime_permission_mode(&state), state.plan_mode);
                }
...
fn prepare_start_turn_state(
    ...
    let previous_wire_mode = claude_wire_mode(runtime_permission_mode(state), state.plan_mode);
...
"#,
    ...
    fn test_prepare_start_turn_stateは同じ構成のprocessを維持してturnを開始する() {
        ...
        let input = TurnInput {
            ...
            permission_mode: runtime_permission_mode(&state),
