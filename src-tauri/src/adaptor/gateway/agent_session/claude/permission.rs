...
use crate::domain::agent_session::services::{
    decide_provider_permission_for_tool, ProviderPermissionDecision,
};
use crate::domain::agent_session::value_objects::JsonPayload;

use super::wire::{ClaudeWireMode, TYPE_CONTROL_RESPONSE};
use super::wire_conversion::permission_mode_and_plan_from_wire;
...
pub(crate) fn should_auto_allow(mode: ClaudeWireMode, tool_name: &str) -> bool {
    let (permission_mode, plan_mode) = permission_mode_and_plan_from_wire(mode);
    decide_provider_permission_for_tool(permission_mode, plan_mode, tool_name)
