...
use crate::domain::agent_session::services::classify_resume_outcome;
...
use super::wire::{
    control_request_subtype, message_subtype, message_type, ClaudeWireMode, DELTA_TEXT,
    DELTA_THINKING, STREAM_CONTENT_BLOCK_DELTA, SUBTYPE_CAN_USE_TOOL, SUBTYPE_PERMISSION_DENIED,
    SYSTEM_COMPACT_BOUNDARY, SYSTEM_INIT, SYSTEM_STATUS, SYSTEM_TASK_NOTIFICATION,
    SYSTEM_TASK_PROGRESS, SYSTEM_TASK_STARTED, SYSTEM_TASK_UPDATED, TYPE_ASSISTANT,
    TYPE_CONTROL_REQUEST, TYPE_CONTROL_RESPONSE, TYPE_KEEP_ALIVE, TYPE_RESULT, TYPE_STREAM_EVENT,
    TYPE_SYSTEM, TYPE_USER,
};
use super::wire_conversion::permission_mode_from_wire;
...
fn convert_system_message(message: &Value, state: &mut ClaudeConvertState) -> ClaudeConversion {
    match message_subtype(message) {
        Some(SYSTEM_INIT) => {
            ...
            let resume = classify_resume_outcome(state.requested_resume_id.as_deref(), &session_id);
