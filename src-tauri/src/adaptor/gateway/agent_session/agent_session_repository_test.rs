...
use crate::usecase::agent_session::{
    AgentSessionLifecycleDto, AgentSessionListRequest, AgentSessionOriginFilter,
...
#[tokio::test]
async fn test_agent_session_repository_create後に再起動して同じ集約を復元できる() {
...
#[tokio::test]
async fn test_agent_session_repository_workflow生成時の初回指示admissionを同時に永続化する() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
...
#[tokio::test]
async fn test_agent_session_repository_provider紐付けと状態遷移を永続化する() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    let session = AgentSession::create(
...
#[tokio::test]
async fn test_agent_session_repository削除をtombstone化してprovider所有権を解放する() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    let first = AgentSession::create(
...
#[tokio::test]
async fn test_agent_session_repository永続化失敗時にprojectionも所有権も進めない() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    let session = AgentSession::create(
...
#[tokio::test]
async fn test_agent_session_query_service同じworkspaceだけをbounded_pageで返す() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    ...
    let query_service = LocalAgentSessionQueryService::new(
        store.clone() as Arc<dyn LocalEventTransactionRepository>
    );
...
#[tokio::test]
async fn test_agent_session_query_service_lifecycleをdata_source側で絞り込む() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    ...
    let page = query_service
        .list(AgentSessionListRequest {
            workspace: WorkspaceIdentity::new("/repo"),
            lifecycle: Some(AgentSessionLifecycleDto::Archived),
            origin: None,
            limit: 10,
            after_session_id: None,
        })
    ...
    assert_eq!(page.items[0].lifecycle, AgentSessionLifecycleDto::Archived);
...
#[tokio::test]
async fn test_agent_session_query_service_standaloneをdata_source側で絞り込む() {
    let directory = TempDir::new().unwrap();
    let store = open_store(&directory);
    let repository = new_repository(&store);
    repository
    ...
    let page = query_service
        .list(AgentSessionListRequest {
            workspace: WorkspaceIdentity::new("/repo"),
            lifecycle: None,
            origin: Some(AgentSessionOriginFilter::Standalone),
            limit: 10,
            after_session_id: None,
        })
        .await
