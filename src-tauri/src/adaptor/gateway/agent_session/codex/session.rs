...
use crate::domain::agent_session::services::{provider_startup_retries, provider_startup_timeout};
use crate::domain::agent_session::value_objects::{EditorContext, ModelId, PermissionMode};
use crate::infrastructure::agent_session::codex::runtime_state::{
    prepare_interrupt_request, reset_completed_turn_state, take_reserved_interrupt_request,
    CodexRuntimeState,
};
...
impl CodexSessionRuntime {
    pub(crate) async fn open(
        ...
        let timeout = provider_startup_timeout(spec.startup_timeout);
        let max_retries = provider_startup_retries(spec.startup_max_retries);
        let mut attempts = 0;
        loop {
...
"#,
    ...
    fn test_startup_timeout既定とclampは旧規則を維持する() {
        ...
        assert_eq!(
            provider_startup_timeout(default_spec.startup_timeout),
            Duration::from_secs(30)
        );
        assert_eq!(
            provider_startup_retries(default_spec.startup_max_retries),
            0
        );
        ...
        assert_eq!(
            provider_startup_timeout(clamped.startup_timeout),
            Duration::from_secs(300)
        );
        assert_eq!(provider_startup_retries(clamped.startup_max_retries), 10);
    }
