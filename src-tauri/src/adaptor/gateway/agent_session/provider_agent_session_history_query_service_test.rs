...
use super::LocalProviderAgentSessionHistoryQueryService;
...
use crate::usecase::agent_session::{
    ProviderAgentSessionHistoryQueryService, ProviderAgentSessionHistoryRequest,
...
impl ProviderAgentSessionHistoryGateway for FixedHistoryGateway {
...
#[async_trait::async_trait]
impl ProviderAgentSessionOwnershipQuery for FixedOwnershipQuery {
    async fn is_owned(
        &self,
        provider: ProviderKind,
...
#[tokio::test]
async fn test_provider_agent_session_history_query_metadataだけを並べ管理中idを除外する() {
    let query = LocalProviderAgentSessionHistoryQueryService::new(
    ...
    assert_eq!(
        page.items
            ...
            .collect::<Vec<_>>(),
        vec![
            (ProviderAgentSessionProviderDto::Codex, "codex-new", 40),
            (ProviderAgentSessionProviderDto::Codex, "codex-old", 20),
        ]
    );
...
#[tokio::test]
async fn test_provider_agent_session_history_query_cursorで次pageをboundedに返す() {
    let query = LocalProviderAgentSessionHistoryQueryService::new(
        Arc::new(FixedHistoryGateway {
            entries: vec![
