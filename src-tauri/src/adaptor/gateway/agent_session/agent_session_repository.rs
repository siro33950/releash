...
use crate::domain::local_event::{
    ...
    AgentSessionProviderRecord, AgentSessionRemovalMutation, CommitBatchError, CommitBatchResult,
    CommitIdentity, CommitOperationKind, ExpectedStreamHead, IdempotencyBinding, LoadStreamRequest,
...
const STORAGE_PREFIX: &str = "agent-session:";
...
impl LocalAgentSessionRepository {
    ...
    async fn create_atomic(
        ...
        let revision_value =
            u64::try_from(domain_events.len()).map_err(|_| AgentSessionRepositoryError::Corrupt)?;
        let revision = Revision::new(
            i64::try_from(revision_value).map_err(|_| AgentSessionRepositoryError::Corrupt)?,
        )
        ...
        let mutation = LocalStateMutation::SessionProjection(SessionProjectionMutation {
            ...
            projection: SessionProjectionRecord::AgentSession(projection(
                &session,
        ...
        let batch = LocalAtomicBatch {
            ...
            idempotency: IdempotencyBinding {
                ...
                idempotency_key: format!("agent-session.create.{caller_request_id}"),
                payload_hash,
            },
            expected_heads,
            events,
    ...
    async fn save_with_lifecycle_events(
        ...
        let batch = LocalAtomicBatch {
            ...
            idempotency: IdempotencyBinding {
                installation_id: self.installation_id.clone(),
                operation_kind: CommitOperationKind::UserMutation,
                idempotency_key: format!("agent-session.save.{caller_request_id}"),
                payload_hash,
            },
            expected_heads,
            events,
        ...
        match self.repository.commit_batch(batch).await {
            ...
            Err(CommitBatchError::PayloadConflict) => Err(AgentSessionRepositoryError::Conflict),
            Err(
                CommitBatchError::StorageUnavailable { .. }
                | CommitBatchError::OutcomeUnknown { .. },
            ) => Err(AgentSessionRepositoryError::Unavailable),
...
impl AgentSessionRepository for LocalAgentSessionRepository {
    ...
    async fn find(
        ...
        let SessionProjectionRecord::AgentSession(projection) = view.projection else {
            return Err(AgentSessionRepositoryError::Corrupt);
        };
        let session = restore(projection)?;
        let revision = u64::try_from(view.revision.value())
            .map_err(|_| AgentSessionRepositoryError::Corrupt)?;
        Ok(Some(VersionedAgentSession::restored(session, revision)))
    }
    ...
    async fn remove(
        ...
        let retained_tombstone_sequence = previous_revision
            .checked_add(1)
            .ok_or(AgentSessionRepositoryError::Corrupt)?;
        mutations.push(LocalStateMutation::AgentSessionRemoval(
...
impl AgentSessionOwnershipQuery for LocalAgentSessionRepository {
    async fn is_owned(
        ...
        .map_err(|error| match error {
            ...
            AgentSessionRepositoryError::AlreadyExists
...
fn restore(
    ...
    let origin = match projection.origin {
        AgentSessionOriginRecord::Standalone => AgentSessionOrigin::Standalone,
    ...
    if projection.lifecycle != AgentSessionLifecycleRecord::Open {
        events.push(AgentSessionLifecycleEvent::LifecycleChanged {
            lifecycle: match projection.lifecycle {
                AgentSessionLifecycleRecord::Open => unreachable!(),
    ...
    AgentSession::rehydrate(&events).map_err(|_| AgentSessionRepositoryError::Corrupt)
}
...
fn provider_kind(provider: AgentSessionProviderRecord) -> ProviderKind {
    match provider {
        AgentSessionProviderRecord::Claude => ProviderKind::Claude,
        AgentSessionProviderRecord::Codex => ProviderKind::Codex,
    }
}

fn origin_record(origin: &AgentSessionOrigin) -> AgentSessionOriginRecord {
    match origin {
        AgentSessionOrigin::Standalone => AgentSessionOriginRecord::Standalone,
...
fn lifecycle_record(lifecycle: AgentSessionLifecycle) -> AgentSessionLifecycleRecord {
    match lifecycle {
        AgentSessionLifecycle::Open => AgentSessionLifecycleRecord::Open,
        AgentSessionLifecycle::Paused => AgentSessionLifecycleRecord::Paused,
        AgentSessionLifecycle::Archived => AgentSessionLifecycleRecord::Archived,
    }
}
