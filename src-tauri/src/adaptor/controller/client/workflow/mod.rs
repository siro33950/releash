...
pub(crate) mod tests {
    use super::execution::{
        ...
        get_workflow_execution_state_impl, get_workflow_node_detail_impl,
        list_workflow_executions_shared as list_workflow_executions,
    };
    use super::*;
    ...
    async fn list_workflow_executions_command_returns_active_first_filtered_by_status() {
        ...
        let all =
            list_workflow_executions(app.state::<AppState>().inner(), None, worktree_path.clone())
                .await
    ...
    async fn list_workflow_executions_canonicalizes_worktree_path_filter() {
        ...
        let executions = list_workflow_executions(app.state::<AppState>().inner(), None, trailing)
            .await
    ...
    async fn get_workflow_execution_command_returns_summary_or_none() {
        ...
        let found = get_workflow_execution(app.state::<AppState>().inner(), execution_id.clone())
            .await
        ...
        let missing =
            get_workflow_execution(app.state::<AppState>().inner(), read_only_test_uuid(99))
                .await
                .expect("get_workflow_execution for unknown execution_id must Ok(None)");
        assert!(missing.is_none());

        let invalid =
            get_workflow_execution(app.state::<AppState>().inner(), "not-a-uuid".to_string()).await;
    ...
    async fn single_execution_read_only_apis_reject_unauthorized_worktree_observation() {
        ...
        let summary = get_workflow_execution(app.state::<AppState>().inner(), execution_id.clone())
            .await
            .expect("get_workflow_execution must succeed");
        assert!(
            summary.is_none(),
