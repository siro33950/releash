...
pub(crate) async fn list_pending_application_attempts_shared(
    journal: &Arc<crate::usecase::application_lifecycle::operation::CallerAttemptJournal>,
    limit: Option<usize>,
...
pub(crate) async fn acknowledge_application_attempt_shared(
    journal: &Arc<crate::usecase::application_lifecycle::operation::CallerAttemptJournal>,
...
pub(crate) async fn request_application_quit_shared(
    process_port: &dyn crate::adaptor::controller::application_lifecycle::ApplicationProcessActionPort,
    ...
    let (dto, process_action) =
        request_application_quit_result(coordinator.as_ref(), request).await?;
    if let Some(process_action) = process_action {
        process_actions.dispatch(process_port, process_action);
    }
    Ok(dto)
}
...
pub(crate) async fn get_application_quit_operation_shared(
    coordinator: &Arc<ShutdownCoordinator>,
    operation_id: String,
) -> Result<ApplicationQuitLookupDtoV1, ApplicationQuitLookupErrorDtoV1> {
    get_application_quit_operation_result(coordinator.as_ref(), operation_id).await
}
...
pub(crate) async fn resolve_shutdown_target_action_shared(
    process_port: &dyn crate::adaptor::controller::application_lifecycle::ApplicationProcessActionPort,
    coordinator: &Arc<ShutdownCoordinator>,
    process_actions: &Arc<
        crate::adaptor::controller::application_lifecycle::ApplicationProcessActionDispatcher,
...
pub(crate) async fn get_application_shutdown_shared(
    coordinator: &Arc<ShutdownCoordinator>,
) -> Result<CurrentShutdownResultDtoV1, CurrentShutdownErrorDtoV1> {
    get_application_shutdown_result(coordinator.as_ref()).await
}
...
pub(crate) async fn get_shutdown_plan_shared(
    coordinator: &Arc<ShutdownCoordinator>,
    shutdown_id: String,
    limit: Option<usize>,
    cursor: Option<String>,
) -> Result<ShutdownPlanPageDtoV1, ShutdownPlanQueryErrorDtoV1> {
    get_shutdown_plan_result(coordinator.as_ref(), shutdown_id, limit, cursor).await
}
...
pub(crate) async fn compact_application_shutdown_details_shared(
    coordinator: &Arc<ShutdownCoordinator>,
    shutdown_id: String,
