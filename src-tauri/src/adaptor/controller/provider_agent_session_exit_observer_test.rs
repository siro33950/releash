...
use crate::usecase::agent_session::{
    ProviderAgentSessionExitPort, ProviderAgentSessionExitUsecase,
...
impl ProviderAgentSessionExitPort for RecordingExitPort {
...
#[tokio::test]
async fn test_provider_agent_session_exit_observer_frontendなしでexitをusecaseへ渡す() {
    ...
    let usecase = Arc::new(ProviderAgentSessionExitUsecase::new(
        Arc::new(FixedTerminalObservation),
        exits.clone(),
    ));
    let task = tokio::spawn(super::run_provider_agent_session_exit_observer(
        stream, usecase,
    ));
