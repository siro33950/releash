...
use crate::usecase::git_host::{PrStatusDto, ProviderStatusDto};

#[tauri::command]
pub async fn check_pr_provider_status(
    state: State<'_, AppState>,
    repo_path: String,
) -> Result<ProviderStatusDto, AppError> {
    let uc = state.git_host_usecase.clone();
    run_blocking(move || ProviderStatusDto::from(uc.check_provider_status(&repo_path))).await
}
