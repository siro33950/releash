...
use crate::usecase::agent_session::ProviderAgentSessionExitUsecase;

pub(crate) async fn run_provider_agent_session_exit_observer(
    mut stream: TerminalSurfaceEventStream,
    usecase: Arc<ProviderAgentSessionExitUsecase>,
) {
    loop {
        match stream.subscription.recv().await {
            Ok(TerminalSurfaceEvent::Exit {
                ...
                if let Err(error) = usecase
                    ...
                    .await
                {
                    log::warn!("failed to observe Provider AgentSession terminal exit: {error:?}");
                }
            }
            Ok(
            ...
            Err(TerminalSurfaceEventReceiveError::Lagged(_)) => {
                ...
                {
                    log::warn!(
                        "failed to reconcile Provider AgentSession terminal exits: {error:?}"
                    );
                }
            }
...
#[cfg(test)]
#[path = "provider_agent_session_exit_observer_test.rs"]
mod provider_agent_session_exit_observer_tests;
