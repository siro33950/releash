...
use super::ProviderAgentSessionActivityEventTap;
...
#[tokio::test]
async fn test_provider_agent_session_activity_tap_出力イベントを観測して通知しつつ転送する() {
    let target = Arc::new(RecordingTargetSink::default());
    let tap = ProviderAgentSessionActivityEventTap::new(target.clone());
    let notifier = Arc::new(RecordingNotifier::default());
    tap.bind(Arc::new(ProviderAgentSessionActivityUsecase::new(
...
#[tokio::test]
async fn test_provider_agent_session_activity_tap_usecase未結合でもイベントを転送する() {
    let target = Arc::new(RecordingTargetSink::default());
    let tap = ProviderAgentSessionActivityEventTap::new(target.clone());
