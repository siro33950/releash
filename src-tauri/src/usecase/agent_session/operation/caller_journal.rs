...
impl CallerAttemptJournal {
    ...
    /// Save the exact command before usecase dispatch. A replay of the same
    /// request identity with the same exact command converges; a different
    /// command under the same identity is a payload conflict.
    #[cfg(test)]
    pub async fn record_attempt(
        ...
        self.record_attempt_scoped(principal, kind, caller_request_id, exact_command, None)
            .await
    }

    pub async fn record_attempt_scoped(
        &self,
        principal: &str,
        kind: OperationKind,
        caller_request_id: &str,
        ...
        if scope_id.is_some_and(|scope| scope.is_empty() || scope.len() > 256) {
            return Err(CallerJournalError::InvalidRequest);
        }
        let command_hash = self.command_hash(exact_command);
    ...
    /// Resolve a journal attempt when one exists. Direct operation replays
    /// can legitimately have no Tauri caller journal, so absence is a closed
    /// no-op. A same-command pending attempt is advanced on every definitive
    /// replay, closing a response-loss window after an earlier resolve write
    /// failed.
    pub async fn resolve_attempt_if_present(
        &self,
        principal: &str,
        kind: OperationKind,
        caller_request_id: &str,
