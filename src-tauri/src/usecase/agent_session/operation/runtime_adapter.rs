//! Agent-session operation runtime drivers.
//!
//! The drivers sequence durable operation usecases, the session store, and
//! the process-local runtime. Canonical payload conversion is injected through
//! a gateway-owned codec.
...
#[cfg(test)]
use crate::adaptor::gateway::agent_session::lifecycle_repository::LocalAgentSessionLifecycleRepository;
#[cfg(test)]
use crate::adaptor::gateway::agent_session::operation::{
    CanonicalSendCommandV1, CanonicalSendTargetV1, CanonicalWorkflowSendPayloadEncoder,
};
use crate::domain::agent_session::aggregates::send_dispatches::{
    SendClaimRegistry, SendDispatchAdmission, SendDispatchRegistry,
};
use crate::domain::agent_session::aggregates::session::TransitionOutcome;
#[cfg(test)]
use crate::domain::agent_session::aggregates::session::{
    QueueItem as AggregateQueueItem, Session as SessionAggregate,
};
#[cfg(test)]
use crate::domain::agent_session::aggregates::session::{
    QueueState as AggregateQueueState, RecoveryFact, SessionRestore,
};
...
use crate::domain::agent_session::services::{
    accepted_send_artifact_digest, accepted_send_retry_delay, accepted_send_target_matches,
    classify_legacy_provider_establish, classify_permission_response_recovery,
    classify_recovery_readback, decide_recovery_action, recovery_handoff_matches,
    session_close_readback_obligation_id, stop_readback_obligation_id,
    workflow_turn_principal_is_authorized, AcceptedSendTarget,
...
use crate::usecase::agent_session::operation::{
    ...
    AgentSendOperationUsecase, BackendRecoveryReadbackPort, BackendRecoveryReadbackRequest,
    LegacyProviderEstablishRecovery, ObligationTransitionOutcome, PermissionResponseEffectPort,
    PermissionResponseExecutionStatus, PermissionResponseOperationUsecase, RecoveryEffectExecutor,
    RecoveryEffectRequest, RecoveryEffectResult, RecoveryOwnerBatch, SendAcceptancePort,
    SendEffectDispatch, SendPlan, SendRecoveryReadbackKind, SendRecoveryReadbackPort,
    ...
    SessionLifecycleEffectPort, SessionLifecycleOperationUsecase, StableRecoveryEffectIdentity,
    StopEffectObservation, StopEffectPort, StopOperationUsecase, StopRecoveryReadbackPort,
...
use crate::usecase::agent_session::session::{
    AcceptedQueuedTurnStartCommitOutcome, NextTurnIdError, SessionStore,
};

#[cfg(test)]
use crate::domain::agent_session::services::INTERNAL_WORKFLOW_OPERATION_PRINCIPAL;
#[cfg(test)]
use crate::usecase::agent_session::operation::LOCAL_INSTALLATION_OPERATION_PRINCIPAL;

#[cfg(test)]
use crate::usecase::agent_session::operation::bind_runtime_terminal_operation_participant_provider;

#[cfg(test)]
fn bind_runtime_durable_workflow_send_driver(
    ...
    crate::usecase::agent_session::operation::bind_runtime_durable_workflow_send_driver(
        ...
        session_store,
        data_dir,
        Arc::new(CanonicalWorkflowSendPayloadEncoder),
    );
}

#[derive(Debug, Clone)]
pub(crate) enum DecodedSendTarget {
    Direct {
...
#[derive(Debug, Clone)]
pub(crate) struct DecodedSendCommand {
    pub target: DecodedSendTarget,
    pub content: String,
...
impl DecodedSendCommand {
    ...
    > {
        let (base_system_prompt, workflow_instructions) = match &self.target {
            ...
            DecodedSendTarget::Direct { .. } | DecodedSendTarget::WorkflowApproval { .. } => {
                (None, Vec::new())
            }
        };
        ...
        let target = match &self.target {
            DecodedSendTarget::Direct {
                ...
                session_id: chat_session_id.as_deref(),
                worktree_path,
            },
            DecodedSendTarget::WorkflowApproval { .. } => AcceptedSendTarget::WorkflowApproval,
            DecodedSendTarget::WorkflowTurn {
                chat_session_id, ..
        ...
        if !accepted_send_target_matches(target, accepted_session_id, accepted_worktree_path) {
            return Err(RuntimeSendOperationAdapter::failure(
...
pub(crate) struct CurrentProcessSendClaims {
    registry: Mutex<SendClaimRegistry>,
}
...
struct CurrentProcessSendDispatches {
    registry: Mutex<SendDispatchRegistry>,
}
...
impl CurrentProcessSendDispatches {
    ...
    fn remove_if_parked(&self, obligation_id: &str) {
        self.registry
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .release_if_parked(obligation_id);
    }
}
...
pub(crate) struct RuntimeSendOperationAdapter {
    runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
    session_store: Arc<SessionStore>,
    data_dir: PathBuf,
    command_codec: Arc<dyn CanonicalSendCommandCodec>,
    ...
    lifecycle_repository:
        Arc<parking_lot::RwLock<Option<Weak<dyn AgentSessionLifecycleRepository>>>>,
    #[cfg(test)]
    _lifecycle_repository_fixture: Arc<dyn AgentSessionLifecycleRepository>,
    #[cfg(test)]
...
#[cfg(test)]
struct TestRuntimeLifecycleRepository {
    runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
    session_store: Arc<SessionStore>,
}
...
impl AgentSessionLifecycleRepository for TestRuntimeLifecycleRepository {
    async fn restore_session(
        &self,
        session_id: &str,
    ) -> Result<SessionAggregate, AgentSessionLifecycleRepositoryError> {
        let response = self
            .runtime
            .get_session(session_id)
            .await
            .map_err(|error| AgentSessionLifecycleRepositoryError::Unavailable(error.to_string()))?
            .ok_or(AgentSessionLifecycleRepositoryError::NotFound)?;
        let recovery_fact = if self
            .session_store
        ...
        let current_turn = response.active_turn_id.map(Turn::start).or_else(|| {
            (response.turn_phase != crate::domain::agent_session::value_objects::TurnPhase::Idle)
                .then(|| Turn::start(1))
        });
        let queue = response
            .pending_queue
            .iter()
            .map(|item| AggregateQueueItem {
                id: item.id.clone(),
            ...
            .collect();
        SessionAggregate::restore(SessionRestore {
            id: session_id.to_string(),
            revision: response.session_revision,
            state: response.session.state,
            has_messages: !response.session.messages.is_empty(),
            has_provider_session: response.session.agent_session_id.is_some(),
            current_turn,
            last_terminal: None,
            queue: AggregateQueueState::restore(queue, response.queue_paused),
            recovery_fact,
        })
...
impl RuntimeSendOperationAdapter {
    #[cfg(test)]
    pub(crate) fn new(
        runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
        session_store: Arc<SessionStore>,
        ...
        Self::new_with_codec(
            ...
            data_dir,
            Arc::new(
                crate::adaptor::gateway::agent_session::operation::CanonicalSendCommandCodecV1,
            ),
        )
    }
    ...
    pub(crate) fn new_with_codec(
        ...
        #[cfg(test)]
        let lifecycle_repository_fixture: Arc<dyn AgentSessionLifecycleRepository> =
            match session_store.local_event_repository() {
                Some(repository) => Arc::new(LocalAgentSessionLifecycleRepository::new(
                    repository,
                    session_store.clone(),
                )),
                None => Arc::new(TestRuntimeLifecycleRepository {
                    runtime: runtime.clone(),
                    session_store: session_store.clone(),
                }),
            };
        #[cfg(test)]
        let lifecycle_repository = { Some(Arc::downgrade(&lifecycle_repository_fixture)) };
        #[cfg(not(test))]
        let lifecycle_repository = None;
        Self {
            runtime,
            session_store,
            data_dir,
            ...
            lifecycle_repository: Arc::new(parking_lot::RwLock::new(lifecycle_repository)),
            #[cfg(test)]
            _lifecycle_repository_fixture: lifecycle_repository_fixture,
            #[cfg(test)]
    ...
    pub(crate) fn bind_lifecycle_repository(
        &self,
        repository: Arc<dyn AgentSessionLifecycleRepository>,
    ) {
        *self.lifecycle_repository.write() = Some(Arc::downgrade(&repository));
    }
    ...
    pub(crate) fn bind_status_sink(
        ...
        self.runtime.set_accepted_send_obligation_driver(Arc::new(
            RuntimeAcceptedSendObligationDriver {
                ...
                lifecycle_repository: self.lifecycle_repository.clone(),
            },
        ));
    }
    ...
    for RuntimeAcceptedSendObligationDriver
{
    ...
    async fn claim_queued_turn_execution(
        ...
        let AgentSessionDomainEvent::TurnStarted { turn_id, .. } = &event else {
            drop(claim);
            self.current_process_send_dispatches.remove(obligation_id);
            return Err(());
        };
        for attempt in 0..ACCEPTED_SEND_RETRY_ATTEMPTS {
            let lifecycle_repository = self
                .lifecycle_repository
                .read()
                .as_ref()
                .and_then(Weak::upgrade);
            let Some(lifecycle_repository) = lifecycle_repository else {
                drop(claim);
                self.current_process_send_dispatches.remove(obligation_id);
                return Err(());
            };
            let mut session = match lifecycle_repository.restore_session(session_id).await {
                Ok(session) => session,
                Err(_) => {
                    drop(claim);
                    self.current_process_send_dispatches.remove(obligation_id);
            ...
            match session.start_queue_head(queue_item_id, Turn::start(*turn_id)) {
...
#[async_trait::async_trait]
impl SendAcceptancePort for RuntimeSendOperationAdapter {
    fn lifecycle_repository(&self) -> Option<Arc<dyn AgentSessionLifecycleRepository>> {
    ...
    async fn plan_send(
        ...
        let command = self.command_codec.decode(canonical_payload)?;
        ...
        // Workflow targets are deliberately resolved only from this adapter.
        // The send usecase performs its existing-operation lookup before
        // invoking it, so response-loss/restart replay never depends on the
        // workflow still being at its original approval checkpoint.
        let (chat_session_id, worktree_path) = match &command.target {
            DecodedSendTarget::Direct {
                chat_session_id,
                worktree_path,
            } => (chat_session_id.clone(), worktree_path.clone()),
            DecodedSendTarget::WorkflowApproval { execution_id } => {
                ...
                (Some(target.chat_session_id), target.worktree_path)
            }
            DecodedSendTarget::WorkflowTurn {
                chat_session_id, ..
            } => {
                if !workflow_turn_principal_is_authorized(principal) {
                ...
                (Some(chat_session_id.clone()), meta.worktree_path)
            }
        };
        ...
        let prompt = crate::domain::agent_session::events::PromptInput {
            ...
            mentions: command.mentions.clone(),
        ...
        Ok(SendPlan {
            ...
            // The gateway carries allocation identities only. The command
            // usecase replaces this proposal with the aggregate's canonical
            // StartedTurn/Queued decision before constructing any event.
            ...
            prompt,
            reserved_turn_id: Some(allocation.next_turn_id.to_string()),
        })
    }
    ...
    async fn classify_legacy_provider_establish(
        ...
        Ok(classify_legacy_provider_establish(
    ...
    async fn start_provider_effect(
        ...
        let command = self.command_codec.decode(&effect.canonical_payload)?;
...
pub(crate) struct RuntimeAgentSessionOperationAdapter {
    runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
    session_store: Arc<SessionStore>,
    data_dir: PathBuf,
...
impl RuntimeAgentSessionOperationAdapter {
    pub(crate) fn new(
        runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
        session_store: Arc<SessionStore>,
        data_dir: PathBuf,
...
#[async_trait::async_trait]
impl SessionLifecycleEffectPort for RuntimeAgentSessionOperationAdapter {
    async fn has_live_runtime(&self, session_id: &str) -> Result<bool, SafeOperationFailure> {
        Ok(self.runtime.has_live_runtime(session_id).await)
    }
    ...
    async fn acceptance_state_mutations(
        &self,
        _session_id: &str,
        _action: &SessionLifecycleAction,
        _events: &[crate::domain::agent_session::events::AgentSessionDomainEvent],
    ) -> Result<Vec<crate::domain::local_event::LocalStateMutation>, SafeOperationFailure> {
        Ok(Vec::new())
    }
...
#[async_trait::async_trait]
impl BackendRecoveryReadbackPort for RuntimeAgentSessionOperationAdapter {
    async fn read_backend_recovery(
        ...
        use crate::domain::agent_session::services::{
            backend_recovery_effect_identity_matches, classify_backend_recovery,
            decide_backend_recovery_readback, BackendRecoveryObservation,
            BackendRecoveryReadbackDecision,
        };

        if !backend_recovery_effect_identity_matches(
        ...
        let observation: BackendRecoveryObservation =
            classify_backend_recovery(&events, &request.recovery_id);
        let participants = if observation.requires_owner_completion() {
            self.session_store
        ...
        let (classification, safe_result) =
            match decide_backend_recovery_readback(observation, participants.is_some()) {
                BackendRecoveryReadbackDecision::Missing => {
                    return Err(Self::failure(
                        "The backend-recovery effect has no matching durable result stream.",
                    ));
                }
                BackendRecoveryReadbackDecision::Pending => (
                    RecoveryResultClassification::Pending,
                    "The backend session recovery remains in progress.".to_string(),
                ),
                BackendRecoveryReadbackDecision::Succeeded => (
                    RecoveryResultClassification::Succeeded,
                    "The backend session recovery completed.".to_string(),
                ),
                BackendRecoveryReadbackDecision::Ambiguous => (
                ...
                BackendRecoveryReadbackDecision::CompleteOwner => {
                    let participants = participants.expect("domain admitted owner completion");
...
mod lifecycle_gate_tests {
    ...
    use super::{RuntimeAgentSessionOperationAdapter, RuntimeSendOperationAdapter};
    use crate::domain::agent_session::repository::AgentSessionLifecycleRepository as _;
    use crate::domain::local_event::{
    ...
    use crate::usecase::agent_session::operation::{
        ...
        SessionLifecycleCommandResult, SessionLifecycleOperationState,
        SessionLifecycleOperationUsecase, SessionLifecycleRequest, StopOperationUsecase,
    };
    ...
    async fn assert_completed_history_idle_action(
        ...
        let lifecycle_snapshot = lifecycle_repository
            .restore_session(&session_id)
            .await
            .unwrap();
        let authority: Arc<dyn OperationBindingAuthority> = store.clone();
        let lifecycle_gate: Arc<
            dyn crate::usecase::agent_session::operation::SessionLifecycleEffectPort,
        ...
        let stop_gate: Arc<dyn crate::usecase::agent_session::operation::StopEffectPort> =
            operation_gate.clone();
        ...
        assert_eq!(lifecycle_snapshot.active_turn_id(), None);
        assert!(
            lifecycle_snapshot.is_quiescent(),
...
#[async_trait::async_trait]
impl StopEffectPort for RuntimeAgentSessionOperationAdapter {
...
pub(crate) struct RuntimePermissionResponseOperationAdapter {
    runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
    #[cfg(test)]
    session_store: Arc<SessionStore>,
}

impl RuntimePermissionResponseOperationAdapter {
    pub(crate) fn new(
        runtime: Arc<crate::usecase::agent_session::runtime::AgentSessionRuntimeUsecase>,
        session_store: Arc<SessionStore>,
    ) -> Self {
...
#[async_trait::async_trait]
impl PermissionResponseEffectPort for RuntimePermissionResponseOperationAdapter {
    async fn request_is_runtime_owned(
        ...
        self.runtime
            .permission_request_is_runtime_owned(session_id, request_id)
            .await
...
fn production_readback_request(
    ...
    let original = immutable_obligation.original();
    let expected_stop_identity = match original {
        ObligationRecord::StopInterrupt {
            session_id,
            turn_id,
            ..
        } => Some(stop_readback_obligation_id(session_id, turn_id)),
        _ => None,
    };
    let expected_session_close_identity = match original {
        ObligationRecord::SessionClose { session_id, .. } => {
            Some(session_close_readback_obligation_id(session_id))
        }
        _ => None,
    };
    match classify_recovery_readback(
        obligation_id,
        original,
        expected_stop_identity.as_deref(),
        expected_session_close_identity.as_deref(),
    )
    .map_err(|_| ())?
    {
        RecoveryReadbackTarget::Stop {
            effect_identity,
            operation_id,
            session_id,
            turn_id,
            ...
            StopRecoveryReadbackRequest {
                effect_identity: StableRecoveryEffectIdentity::parse(effect_identity)?,
                operation_id,
                session_id,
                turn_id,
            },
        )),
        RecoveryReadbackTarget::SessionClose {
            effect_identity,
            operation_id,
            session_id,
        ...
        RecoveryReadbackTarget::BackendRecovery {
            effect_identity,
            session_id,
            recovery_id,
        ...
        RecoveryReadbackTarget::SendTurn {
            effect_identity,
            operation_id,
            session_id,
...
impl RecoveryEffectExecutor for ConservativeRecoveryExecutor {
    ...
    async fn execute(
        ...
        let readback =
            production_readback_request(&request.obligation_id, &request.immutable_obligation).ok();
        let observation = request
            .authoritative_observation
            .as_ref()
            .map(|observation| (observation.classification, observation.cancellable));
        ...
        .map_err(|rejection| {
            let label = match rejection {
                RecoveryActionRejection::UnsupportedReadback => {
                    "This obligation does not have a supported typed readback."
                }
                RecoveryActionRejection::MissingAuthoritativeObservation => {
                    "A verified backend observation is required for this recovery action."
                }
                RecoveryActionRejection::PermissionOperationIdentityUnavailable => {
                    "The saved permission operation identity is unavailable."
                }
                RecoveryActionRejection::MissingCancellationProof => {
        ...
        match decision {
            RecoveryActionDecision::ReadAgain => {
                match readback.expect("domain admitted readback") {
            ...
            RecoveryActionDecision::UseObservedResult => {
                let observation = request
                    .authoritative_observation
                    .as_ref()
                    .expect("domain admitted authoritative observation");
                Ok(RecoveryEffectResult {
            ...
            RecoveryActionDecision::KeepForManualResolution => Ok(RecoveryEffectResult {
                classification: C::Unchanged,
                safe_result: "The obligation was retained for manual resolution.".to_string(),
                owner_mutations: Vec::new(),
                owner_batch: None,
            }),
            RecoveryActionDecision::RetryPermissionResponse { operation_id } => {
                let operation = self
                    .permission_response
                ...
                let (observation, safe_result) = match operation.latest_status {
                    PermissionResponseExecutionStatus::Completed { .. } => (
                        PermissionResponseRecoveryObservation::Completed,
                        "The exact permission response was delivered.",
                    ),
                    PermissionResponseExecutionStatus::AwaitingProviderResponse { .. } => (
                        PermissionResponseRecoveryObservation::AwaitingProviderResponse,
                        "The saved permission response remains pending.",
                    ),
                ...
                Ok(RecoveryEffectResult {
                    classification: classify_permission_response_recovery(observation),
                    safe_result: safe_result.to_string(),
                    owner_mutations: Vec::new(),
                    owner_batch: None,
                })
            }
            RecoveryActionDecision::CancelConfirmedNoEffect => Ok(RecoveryEffectResult {
                classification: C::CancelledBeforeEffect,
                safe_result: "The effect was confirmed absent and safely cancelled.".to_string(),
                owner_mutations: Vec::new(),
                owner_batch: None,
            }),
        }
...
mod send_execution_tests {
    ...
    use super::{
        ...
        CurrentProcessSendDispatches, RuntimeSendOperationAdapter, ACCEPTED_SEND_RETRY_ATTEMPTS,
    ...
    fn runtime_send_gate_constructor_is_safe_without_an_entered_tokio_runtime() {
        ...
        let _gate =
            RuntimeSendOperationAdapter::new(runtime, session_store, data.path().to_path_buf());
    }
    ...
    async fn accepted_send_claim_reply_loss_preserves_the_exact_winner_without_manual_recovery() {
        ...
        let gate = Arc::new(RuntimeSendOperationAdapter::new(
            Arc::clone(&runtime),
            Arc::clone(&session_store),
            data_path.clone(),
        ));
        ...
        assert!(
            crate::usecase::agent_session::operation::SendAcceptancePort::owns_current_process_turn_execution(
                gate.as_ref(),
                &session_id,
                operation_id,
    ...
    async fn run_b006_production_queued_restart(
        ...
        let restarted_gate = Arc::new(RuntimeSendOperationAdapter::new(
...
mod workflow_send_execution_tests {
    ...
    use super::{
        bind_runtime_durable_workflow_send_driver, RuntimeSendOperationAdapter,
...
mod stop_execution_tests {
    ...
    use super::{
        bind_runtime_terminal_operation_participant_provider, RuntimeAgentSessionOperationAdapter,
        RuntimeSendOperationAdapter, LOCAL_INSTALLATION_OPERATION_PRINCIPAL,
    };
...
mod recovery_executor_tests {
    ...
    use super::{
        ...
        RuntimeAgentSessionOperationAdapter, RuntimePermissionResponseOperationAdapter,
        RuntimeSendOperationAdapter,
    };
    ...
    use crate::usecase::agent_session::operation::{
        ...
        PendingRecoveryQuery, PermissionResponseEffectPort, PermissionResponseOperationUsecase,
        RecoveryActionError, RecoveryActionOutcome, RecoveryActionRejection, RecoveryActionRequest,
        RecoveryActionResultOutcome, RecoveryActionStatus, RecoveryActionUsecase,
        RecoveryEffectExecutor, RecoveryEffectRequest, RecoveryEffectResult,
        SendRecoveryReadbackPort, SendRecoveryReadbackRequest, SessionCloseRecoveryReadbackPort,
        SessionCloseRecoveryReadbackRequest, SessionLifecycleOperationUsecase, StopCommandOutcome,
        StopEffectObservation, StopEffectPort, StopOperationRequest, StopOperationState,
        ...
        for UnusedPermissionGate
    {
        ...
        > {
            Err(
                crate::domain::agent_session::repository::AgentSessionLifecycleRepositoryError::NotFound,
            )
        }
    ...
    #[async_trait::async_trait]
    impl PermissionResponseEffectPort for UnusedPermissionGate {
        async fn request_is_runtime_owned(
            &self,
            _session_id: &str,
            _request_id: &str,
    ...
    fn production_executor_fixture() -> (
        ...
        .unwrap();
        let permission_gate = Arc::new(UnusedPermissionGate);
    ...
    #[async_trait::async_trait]
    impl StopEffectPort for F05ResultLostStopGate {
        async fn interrupt(
            &self,
    ...
    async fn f05_production_backend_recovery_read_again_finishes_from_durable_owner_evidence_after_restart(
        ...
        // the concrete production RuntimeAgentSessionOperationAdapter.
        drop(session_store);
        drop(store);
        let reopened = crate::adaptor::gateway::local_event_store::LocalEventStore::open(
            crate::adaptor::gateway::local_event_store::LocalEventStoreConfig::production(
    ...
    struct AtomicProductionReadbackPorts {
        ...
        backend: Arc<RuntimeAgentSessionOperationAdapter>,
