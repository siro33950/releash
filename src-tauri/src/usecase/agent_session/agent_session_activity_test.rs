...
use super::{AgentSessionActivityUsecase, AgentSessionChangeNotifier};
...
struct ActivityFixture {
    usecase: AgentSessionActivityUsecase,
...
fn fixture(session_key: &str, worktree_path: &str) -> ActivityFixture {
    ...
    ActivityFixture {
        usecase: AgentSessionActivityUsecase::new(
            terminal.clone(),
            notifier.clone(),
...
async fn test_agent_session_activity_idleからrunningエッジで通知する() {
    let ActivityFixture {
        usecase,
        notifier,
        terminal,
...
#[tokio::test(start_paused = true)]
async fn test_agent_session_activity_無出力3秒のdebounceでidle遷移を通知する() {
    let ActivityFixture {
        usecase, notifier, ..
    } = fixture("session-key", "/repo/worktree");
...
#[tokio::test(start_paused = true)]
async fn test_agent_session_activity_出力継続中はdebounceをリセットしてidle遷移しない() {
    let ActivityFixture {
        usecase, notifier, ..
    } = fixture("session-key", "/repo/worktree");
...
#[tokio::test(start_paused = true)]
async fn test_agent_session_activity_session所有でないsurfaceは追跡しない() {
    let ActivityFixture {
        usecase,
        notifier,
        terminal,
...
#[tokio::test(start_paused = true)]
async fn test_agent_session_activity_running中のexitはidle遷移として通知する() {
    let ActivityFixture {
        usecase, notifier, ..
    } = fixture("session-key", "/repo/worktree");
