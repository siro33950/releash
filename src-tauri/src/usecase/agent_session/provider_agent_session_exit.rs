...
use super::{ProviderAgentSessionLifecycleUsecase, ProviderAgentSessionLifecycleUsecaseError};

#[async_trait::async_trait]
pub(crate) trait ProviderAgentSessionExitPort: Send + Sync {
...
pub(crate) struct ProviderAgentSessionExitUsecase {
    terminal: Arc<dyn ProviderAgentTerminalObservationGateway>,
    sessions: Arc<dyn ProviderAgentSessionExitPort>,
}

impl ProviderAgentSessionExitUsecase {
    pub(crate) fn new(
        terminal: Arc<dyn ProviderAgentTerminalObservationGateway>,
        sessions: Arc<dyn ProviderAgentSessionExitPort>,
    ) -> Self {
    ...
    pub(crate) async fn observe_exit(
        ...
        runtime_generation: u64,
        caller_request_id: &str,
    ) -> Result<bool, ProviderAgentSessionLifecycleUsecaseError> {
        let Some(owner) = self
            .terminal
    ...
    pub(crate) async fn reconcile_exited(
        &self,
        caller_request_id: &str,
    ) -> Result<usize, ProviderAgentSessionLifecycleUsecaseError> {
        let mut observed = 0;
...
#[async_trait::async_trait]
impl ProviderAgentSessionExitPort for ProviderAgentSessionLifecycleUsecase {
    async fn observe_process_exit(
        &self,
        agent_session_id: &str,
        runtime_generation: u64,
        ...
        ProviderAgentSessionLifecycleUsecase::observe_process_exit(
            self,
            agent_session_id,
            runtime_generation,
            exit_code,
