...
use super::{ProviderAgentSessionExitPort, ProviderAgentSessionExitUsecase};
...
impl ProviderAgentSessionExitPort for RecordingExits {
    ...
    ) -> Result<(), super::ProviderAgentSessionLifecycleUsecaseError> {
...
#[tokio::test]
async fn test_provider_agent_session_exit_session所有surfaceだけを通知する() {
    ...
    let usecase = ProviderAgentSessionExitUsecase::new(terminal, exits.clone());
...
#[tokio::test]
async fn test_provider_agent_session_exit_lagged後にexited_surfaceを再照合する() {
    let exits = Arc::new(RecordingExits::default());
    let terminal = Arc::new(TerminalObservations {
        owner: Some((
            1,
...
#[tokio::test]
async fn test_provider_agent_session_exit_旧runtime世代の遅延exitで現行sessionを停止しない() {
    let exits = Arc::new(RecordingExits::default());
    let terminal = Arc::new(TerminalObservations {
        owner: Some((
            8,
