...
pub(crate) struct AgentSessionActivityUsecase {
    terminal: Arc<dyn ProviderAgentTerminalObservationGateway>,
    notifier: Arc<dyn AgentSessionChangeNotifier>,
    runtime: tokio::runtime::Handle,
    entries: Arc<Mutex<HashMap<String, TrackedSurface>>>,
}

impl AgentSessionActivityUsecase {
    pub(crate) fn new(
        terminal: Arc<dyn ProviderAgentTerminalObservationGateway>,
        notifier: Arc<dyn AgentSessionChangeNotifier>,
        runtime: tokio::runtime::Handle,
    ...
    pub(crate) fn observe_output(&self, session_key: &str) {
        ...
        self.notifier.agent_session_changed(&worktree_path);
    ...
    pub(crate) fn observe_exit(&self, session_key: &str) {
        ...
        if let Some(TrackedSurface::Session(entry)) = removed {
            if entry.running {
                self.notifier.agent_session_changed(&entry.worktree_path);
            }
        }
    }
        ...
        self.runtime.spawn(async move {
            ...
            loop {
                ...
                if let Some(worktree_path) = transitioned {
                    notifier.agent_session_changed(&worktree_path);
                    return;
                }
            }
        });
...
#[cfg(test)]
#[path = "agent_session_activity_test.rs"]
mod agent_session_activity_tests;
