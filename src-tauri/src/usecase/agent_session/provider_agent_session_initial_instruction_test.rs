...
use super::provider_agent_session_initial_instruction::ProviderAgentInitialInstructionOutcome;
use super::{ProviderAgentInitialInstructionUsecase, ProviderAgentSessionUsecase};
...
#[tokio::test]
async fn test_provider_agent_initial_instruction_session操作lock解放後に送る() {
    ...
    let usecase = Arc::new(ProviderAgentInitialInstructionUsecase::new(
        sessions.clone(),
        terminal.clone(),
    ));
    ...
    assert_eq!(
        ...
        ProviderAgentInitialInstructionOutcome::DeliveryUnknown
    );
}

#[tokio::test]
async fn test_provider_agent_initial_instruction_delivery不明でも永続化後に一度だけ送る() {
    let directory = tempfile::tempdir().unwrap();
    let store = LocalEventStore::open(LocalEventStoreConfig::production(
        directory.path().to_path_buf(),
    ))
    ...
    sessions
        ...
        .unwrap();
    let terminal = Arc::new(FailingTerminalInput::default());
    let usecase = ProviderAgentInitialInstructionUsecase::new(sessions.clone(), terminal.clone());
    ...
    assert_eq!(
        usecase
            ...
            .await
            .unwrap(),
        ProviderAgentInitialInstructionOutcome::AlreadyDispatched
    );
    assert_eq!(
