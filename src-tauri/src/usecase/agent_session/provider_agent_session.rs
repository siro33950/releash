...
pub(crate) enum ProviderAgentSessionUsecaseError {
    NotFound,
    InvalidOperation,
    Conflict,
...
pub(crate) struct ProviderAgentSessionCreateRequest {
...
pub(crate) struct ProviderAgentSessionUsecase {
    repository: Arc<dyn ProviderAgentSessionRepository>,
    operation_locks: Mutex<HashMap<String, Weak<AsyncMutex<()>>>>,
}

impl ProviderAgentSessionUsecase {
    pub(crate) fn new(repository: Arc<dyn ProviderAgentSessionRepository>) -> Self {
        Self {
            repository,
    ...
    ) -> Result<OwnedMutexGuard<()>, ProviderAgentSessionUsecaseError> {
        let lock = {
            let mut locks = self
                .operation_locks
                .lock()
                .map_err(|_| ProviderAgentSessionUsecaseError::Corrupt)?;
    ...
    pub(crate) async fn create_with_lifecycle_events(
        &self,
        request: ProviderAgentSessionCreateRequest,
    ...
    pub(crate) async fn find(
        &self,
        agent_session_id: &str,
    ) -> Result<Option<VersionedProviderAgentSession>, ProviderAgentSessionUsecaseError> {
        self.repository
    ...
    pub(crate) async fn prepare_provider_session_association(
        ...
        let mut session = self
            ...
            .await
            .map_err(map_repository_error)?
            .ok_or(ProviderAgentSessionUsecaseError::NotFound)?;
        session
            .session_mut()
    ...
    pub(crate) async fn observe_process_exit(
        ...
        caller_request_id: &str,
    ) -> Result<AgentSessionProcessExitOutcome, ProviderAgentSessionUsecaseError> {
    ...
    pub(crate) async fn complete_resume(
        ...
        caller_request_id: &str,
    ) -> Result<AgentSessionMutationOutcome, ProviderAgentSessionUsecaseError> {
        let mut session = self.required(agent_session_id).await?;
        let outcome = session
            .session_mut()
    ...
    pub(crate) async fn archive(
        ...
        agent_session_id: &str,
        caller_request_id: &str,
    ) -> Result<AgentSessionArchiveOutcome, ProviderAgentSessionUsecaseError> {
        let mut session = self.required(agent_session_id).await?;
        let outcome = session
            .session_mut()
            .archive()
    ...
    pub(crate) async fn delete(
        ...
        agent_session_id: &str,
        caller_request_id: &str,
    ) -> Result<(), ProviderAgentSessionUsecaseError> {
    ...
    pub(crate) async fn admit_initial_instruction(
        ...
        agent_session_id: &str,
        caller_request_id: &str,
    ) -> Result<AgentSessionInitialInstructionOutcome, ProviderAgentSessionUsecaseError> {
        let mut session = self.required(agent_session_id).await?;
        let outcome = session
            .session_mut()
            .admit_initial_instruction()
...
fn map_repository_error(
    ...
    match error {
        ...
        ProviderAgentSessionRepositoryError::ProviderSessionAlreadyOwned { agent_session_id } => {
            ProviderAgentSessionUsecaseError::ProviderSessionAlreadyOwned { agent_session_id }
        }
        ProviderAgentSessionRepositoryError::InvalidRequest => {
            ProviderAgentSessionUsecaseError::InvalidOperation
        }
        ProviderAgentSessionRepositoryError::Corrupt => ProviderAgentSessionUsecaseError::Corrupt,
