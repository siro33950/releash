...
- 指摘がない場合: 確認結果を簡潔にまとめ、出力Contractに `verdict: "LGTM"` と `findings: []` を提出する
- 指摘がある場合: 上記フォーマットの指摘リストを整理し、出力Contractに `verdict: "NEEDS_FIX"` と `findings` を提出する
- `findings` には Thread ID、対象ファイル、問題、期待、現状を入れる
