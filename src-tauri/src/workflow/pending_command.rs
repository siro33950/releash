...
//! 稼働中アプリの watcher が pickup し、`WorkflowEngine::dispatch_cli_pending`
//! 経由で typed `WorkflowCommand` に変換 + dispatch する（spec [06] アーキ
//! テクチャ概要 / 責務配置）。
//!
//! 本モジュールの責務:
...
//!
//! 担当しない: engine 内部 state mutation、`WorkflowCommand` 組み立て、
...
/// dispatcher adapter が本 payload を `WorkflowCommand` と event log 用 request の
/// 両方へ明示変換する。
