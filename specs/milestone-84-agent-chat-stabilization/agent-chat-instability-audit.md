...
- [d3-durable-event-store-design.md](d3-durable-event-store-design.md) — durable event storeの物理設計gate
...
次の所見は完了済みPhase 0保証の受入条件を破るため、個別Issueへ分けずPhase 0 completion [#1499](https://github.com/siro33950/releash/issues/1499)で一括して閉じる。#1499は本節の全行、D3 physical design gate、close / quit decision table、統合fault matrixが揃ったときだけ完了する。
...
| future local atomic event storeのengine、schema、WAL、migration、backup、performanceが未決でPhase 0 bridgeの引き渡し先がない | L-P6 / D3 gate不在 | `d3-durable-event-store-design.md`の13 decision |
