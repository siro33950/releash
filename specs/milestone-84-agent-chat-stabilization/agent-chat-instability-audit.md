...
milestone 84 のドキュメント群（本書が要求リスト、以下5書が理想形・設計判断の正本）:
...
- [close-quit-decision-table.md](close-quit-decision-table.md) — close / quit surfaceの正本
...
次の所見は完了済みPhase 0保証の受入条件を破るため、個別Issueへ分けずPhase 0 completion [#1499](https://github.com/siro33950/releash/issues/1499)で一括して閉じる。#1499は本節の全行、F2型統合、恒久SQLite store、Legacy→SQLite migration、close / quit decision table、統合fault matrixが揃ったときだけ完了する。

2026-07-21の設計統合で、F2 #1384とF3 #1385を#1499へ内包し、暫定file-store closureから将来SQLiteへ渡す二段構成を廃止した。本節の監査所見と利用者可視invariantは維持するが、表内のPhase 0 root / manifest / closure / materializer、bootstrap、将来F3、3-tree COW physical type、D3 gateという実装処方は[Issue #1499 Primary Spec](../../docs/specs/issues-1499/design.md)のdomain型統合、恒久SQLite transaction / table、Legacy→SQLite migrationへ置き換える。`phase0_*`をruntime module / type / table名として実装せず、cutover後はSQLite-onlyとする。GitHub Issue / milestoneの管理更新はこの設計統合と分離して後で行う。

| 追補所見 | 根因 / invariant | #1499の解消契約 |
|---|---|---|
| 通常sendにstable operation identityがなく、response喪失・query失敗・restart後の再送が別message / turn / Sessionを作る | command acceptanceのdurable authority不在 / I6 | 既存`send_agent_message`にWebSocket outer request IDとは別のcaller指定opaque `SendOperationId`をadditiveに追加する。IDは1〜128 bytesの`[A-Za-z0-9._:-]`でcurrent-installation principal scope内に限定する。Rustが現行入力`chat_session_id / worktree_path / permission_mode / plan_mode / backend_id / model_id`＋content / images / mentions / editor context / active-turn policyのexact binding、validation、durable intent、immutable receipt（opaque `input_ref`必須）、mutable status、retryを所有する。同じprincipal / ID / payloadは同decisionをreplayし、異payloadはsend resultやexact 19種のSafeOperationFailureではなくWebSocket / Tauri共通の`PayloadConflict { identity: Send { operation_id } }`としてeffect 0件で返す。writer開始前failureは`RejectedBeforeCommit`、writer開始後の保存結果不明はembedded `OutcomeUnknown`として同identityのquery / same-payload retryへ収束させる。Accepted後のprovider未解決はtop-level Acceptedを維持した`SendExecutionStatus::ReconciliationRequired`とし、別operationを自動sendしない。public Prepare / Commit / Cancel、prepared owner list、prepared content resolverは作らない |
| active-turn fallbackでAcceptedになったqueued inputがmemory-onlyで、restart後に同じreceiptを再構築できない | I4 / #1404 non-goalとの境界不整合 | receipt-linkedな`QueueExecution(Pending)`をacceptance manifestへ同居させる最小durable queue acceptance。queue item / execution identity / acceptance時採番のreserved turn ID / snapshot hash、provider start用の必須ExternalEffectIntent、既存runtime identity / epoch / effective configuration hashまたはpredeclared ProviderEstablish identityを表すclosed `QueueRuntimeGuard`を固定し、pending turn IDとdrain / terminal umbrella lookupも同じreserved IDを使う。drainはdependency successとguard全fieldのbyte一致後だけclaim / EffectReservedへ進め、immutable payloadへintent / guard / ProviderEstablish dependencyを後付けしない。acceptance後のruntime喪失、runtime / configuration drift、guard不一致、新establish必要時はeffect 0件のままsame QueueExecutionをReconciliationRequired、queueをPausedへ進める。#1499の実装範囲はPending / intent / guard / dispatch handoffまでであり、provider start前cancelを含むqueue cancel / CAS / rebase transitionと`CancelledBeforeEffect`確定は#1404だけが所有する。full queue lifecycleは#1404 |
| #1499導入前のlegacy Session / turn / queue / permissionにPhase 0 authority recordがなく、空rootから開始すると既存未完了workを消失または再実行する | 初回upgrade bootstrap / authority cutover不在 | 明示migration不要のautomatic bootstrapをexclusive writer lock＋normal mutation / effect admission閉鎖で実行する。immutable legacy inventoryはread最大200 source logical records / decoded source 16 MiBで読み、staged payloadを合算しない。staged candidateは別physical K tier最大17 MiB、normal lane 64 MiBへ収める。一sourceをdeterministic substep microcommitへ分割してもcursor / imported countは全substepとsource parity finalize後だけ進め、partialをpublic projection / parity authorityへ出さない。crash後はsame source / substepから再開し、stagingをnon-authorityに保つ。terminal / permission / unknown raw bytesを保持、証明不能active turnをReconciliationRequired、guard不能queueをPaused＋ReconciliationRequiredへ写す。局所破損は最小scope quarantineにし、exact legacy raw bytesをprivate immutable blobへ1 MiB chunk / step 16 MiBでcrash-resumableに保存し、final byte length / SHA-256一致後だけrecordをcommitする。ordered quarantined raw bytes hashをparityへ含め、public viewにblob ref / bytesを出さず、legacy sourceのparity検証、Phase 0 activation、F3 one-shot importに必要なretentionが完了するまで保持する。parityはlegacy画面の旧表示との文字列一致ではなく、同じlegacy bytesへclosed normalization rulesを適用したexpected normalized message / permission / queue projectionとの一致にする。確定済みtranscript / terminal / owner relationを維持し、証明不能active / queue / permissionはReconciliationRequired / Paused＋ReconciliationRequired / Failedまたはquarantineへ保守変換して比較する。source / staged parityとrequired sync後のauthority pointer CASだけをcutoverとし、切替後はlegacy read-only、dual write / per-record fallback / query merge / rollbackを禁止する。bootstrap queryはInspectingSource / Importing / Verifying / Activating / Failed、finalized count、read-only、Failedだけのsafe failureをTauri / WebSocketで同一投影し、pointer / root / parity検証、reachable replay / pending inventory validation、normal admission open後だけNoneにする。bootstrap中のquitはnormal plan / target /明示commandを作らずcurrent step / pointer attemptを13秒までsettleして15秒one-shot exitし、OutcomeUnknownやimplicit exitをterminal / no-effect / migration successへ推測しない。physical identity helperはu16 BE count / u32 BE length / u64 BE unsigned numeric / closed ASCII enum / raw UTF-8で1..=16 components、各1..=1024 bytes、canonical input最大16 KiB、logical key 1..=1024 bytesをhard rejectし、signed numeric、truncate、decimal ASCII、raw path連結、別hash fallbackを許さない |
| canonical mutation失敗をretry後にwarn / transient noticeだけで続行するpathが残る | L-P1 / L-P3 / I8 | mutating path全数分類、pre-effect rejection、post-effect reconciliation。turn / queue、permission、provider establish・resume、terminal、recovery、Session close、Workflow shutdownのexternal effectをpre-existing obligation→EffectReserved後にだけ実行する。canonical stateはPending recordとcompact Terminal resultのclosed enumで、pendingだけがpayload / dependency / claim generation / private refを持つ。Terminal transitionはpending index / claimを削除し、owner / association / kind / 4値result / safe observation・failure / completed_at / revisionだけへcompactする。dependencyはcreation後immutableなfixed-kind DAG / max depth 1とし、Turn / Queue→同owner・同operationのdistinct ProviderEstablish 0..=3、RecoveryPublication→同recoveryのBackendRecovery exact 1、他kind→空だけを許す。creation / claimは最大3 direct lookupで検証し、任意graph traversalを行わず、`Terminal(Succeeded)`だけで満たす。runtime未確立sendはacceptance時にProviderEstablishも予約してそのSucceeded前にTurn / Queueをclaimしない。manual actionも第5のgeneric resultを作らない。BackendRecovery payloadはprovider recovery effectとpublication obligation IDを必須所有し、RecoveryPublicationだけがmessage identity / payload / publication lifecycleを所有する。RecoveryPublicationはexternal effectではなくBackendRecovery completionでPendingとなるlocal publication obligationで、local claim後もEffectReservedを通らずmessage＋marker＋Terminalを同closureで確定する。permission exact payloadはowner-only private refでObserved / terminalまで保持し、terminal commit後に他のrefが無ければGCする |
| `LocalPersistenceFailureKind::OutcomeUnknown`とstructured transaction / closure OutcomeUnknownが併存すると、writer開始後のunknownを`BeforeCommit(LocalPersistenceFailure)`へ格下げできる | commit前failureとcommit成否不明の型境界欠落 | `LocalPersistenceFailureKind`からOutcomeUnknownを除去する。`LocalEventTransactionError::OutcomeUnknown { transaction_id, payload_hash }`は`commit_batch` writer開始後だけ生成し、read / snapshot / watchは生成しない。Phase 0 closureも同じ2 fieldのstructured variantだけを使い、PersistFailure / BeforeCommitへ包まずsame transaction identityのbounded lookupで解決する |
| compact Terminalがoperation / recovery IDとdependencyを捨てるため、claim時に「同owner・同operation」のdependencyだったことを再検証できない | compactionとfixed dependency gateの情報境界不整合 | Pending / Terminal双方にdomain-separated kind / owner / association / kind-specific stable IDsの`semantic_correlation_sha256`を保持する。Turn / Queueはstructured operation generation＋value、ProviderEstablishは同じgeneration＋value＋launch / recovery IDを含め、send由来3 kindはgeneric operation_id=Someを必須とする。Pendingにはordered dependency IDsとsame-length max3のbinding hashを固定し、creation / importでparent / ProviderEstablishのoperation generation＋value byte一致、relation、bindingを検証する。claimは最大3 direct Terminal lookupでbinding hash一致＋Succeededだけを受理する。Terminalは自身のcorrelation hashだけを残しpayload / dependency listを復元しないため、compact済みProviderEstablishを別operationへ流用できない。不一致はeffect 0件でquarantineする |
| effect capabilityとidempotency / correlation keyの組合せが未検証で、安全でないRetry actionを提示し得る | provider effect再実行のproof不在 | 全intentのcorrelation keyをUTF-8 1..=512 bytes、IdempotentRetry系はstable idempotency key 1..=512 bytes必須、AuthoritativeReadback系はprovider-native stable correlation必須、Noneはidempotency keyなし・auto retry/readback禁止とする。common builder / import / claim validatorでinvalid pairをeffect前rejectまたはquarantineする |
| pending / shutdown projectionがRust-owned actionsを返すが共通commandとresponse-loss identityがなく、logical Prepared shutdown targetを解決してplanをterminal化できない | recovery control plane / action commit point不在 | Tauri / WebSocket共通`resolve_pending_recovery_action` / `resolve_shutdown_target_action`を定義し、backend発行opaque action IDをstable identityにする。closed resultは`Completed { result } / InProgress { action_id } / Rejected { NotFound / RevisionConflict { current_revision } / ActionUnavailable / TargetRevisionChanged } / ActionOutcomeUnknown { action_id }`だけである。commandはcurrent resource / plan detailsより先にdurable decisionをlookupし、same actionのCompletedは保存済みreceipt、result classification、canonical safe resultをexact replayする。nonterminalはsame attemptへjoinし、writer unknownはActionOutcomeUnknownとして同transactionをresolveしWS errorへ複製しない。decision Absentのfresh actionだけがcurrent revision / root / state guardを検証する。ReadAgain / RetrySameEffectはclaim / reservationをwrite-ahead後にexternal I/Oへ進み、UseObservedResult / CancelIfSafe / KeepForManualResolutionはkind-specific side stateとCompleted receiptを同じclosureで確定する。response喪失したsame actionをstale conflictにせず、異actionのguard不一致だけをeffect 0件のRevisionConflict / ActionUnavailable / TargetRevisionChangedとする |
| recovery actionのresponse喪失後に別action identityから同じexternal effectを予約できると二重作用になる | action receipt identityとeffect一回性のfenceが同一視されている | external `ReadAgain / RetrySameEffect`はcanonical `RecoveryActionResourceRef`と保存済みeffect identityへ束縛したeffect fenceをAttempt / claim / reservationと同じclosureでwrite-aheadする。same action IDはcurrent resourceより先にdurable decisionをlookupし、Completedをexact replay、nonterminalをsame attemptへjoin、OutcomeUnknownをsame transactionへ解決する。別actionが同じresource / effect identityへ到達してもmatching fenceから既存Attemptへjoin / readbackし、new Attempt / effectを0件にする。fenceはAttempt terminalかつresource state前進の証明と同じclosureでだけ削除し、client入力やaction tokenだけからeffect identityを生成しない |
| resource-isolated input、operation / resource privacy purge、managed backup / restore、app-data resetをPhase 0へ同時導入するとsend安定化とdata lifecycle設計の境界が崩れる | D3 / F3後続基盤への依存 | #1499のPhase 0 runtime contractから除外する。必要な公開要件、data lifecycle、migration、physical reclamationはD3 / F3の後続設計で別途確定し、Phase 0 send / recovery / shutdown契約へ暗黙追加しない。complete privacy authority reset / exported copy importもunsupportedとする |
| recovery actionがobligation resultだけをcompactすると、turn / permission / publication / Session / Workflow等のowner side stateがpendingのまま残る | action terminal authorityの分断 | generic result-only terminal pathを禁止する。開始 / ack proofだけならpending＋proof ref / safe observation＋owner / operation status＋attempt Completed receipt / safe result（outcome=Pending）を同closureで更新する。terminal proof時だけTurn / Queue / TerminalCommit→R-020 canonical terminal、ProviderEstablish→dependency / send status、PermissionDelivery→permission settlement、BackendRecovery→RecoveryPublication Pending handoff、RecoveryPublication→message publication、SessionClose→Session / configuration / archive、WorkflowShutdown→workflow stateのkind-specific closureへ委譲し、compact result＋claim / index / fence削除＋proof由来safe observation digest＋side state＋attempt Completedを同commitにする。side participant failure時は元pending identityを保持する。QueueExecution CancelIfSafeは#1404のため提示しない |
| obligationがAbsentのlogical shutdown target actionでcompact resultだけを作るとSession / Workflow owner projectionとplan targetが分裂する | logical target closureのside-state欠落 | non-sensitive page refからtarget authorityをpoint lookupし、digest、plan / epoch / page index / target key / ordinal / obligation ID / expected revisionをbyte検証する。authority欠損または不一致はraw owner / effectを再構築せずCorruptとし、actionを出さない。valid authorityのsame deterministic IDを使い、Succeeded / ConfirmedNoEffectはSessionClose / WorkflowShutdown owner projection＋compact result＋proof由来safe observation digest＋action attempt Completed receipt / safe result＋cleanupを同じtarget closureで確定する。AmbiguousはPending(ReconciliationRequired)＋bounded reason＋claim state＋scope fenceと、readback action自体の正常完了を示すaction attempt Completed receipt / canonical safe result（outcome=Pending）を同じclosureで確定しsame action IDへexact replayする。attempt ReconciliationRequiredはaction自身のresult / commitを確定できない場合だけとする。idempotent retryもEffectReserved＋claim＋pending index＋fence＋top-level claim generation Someのattemptをwrite-aheadで同時確定する。owner / side participant failure時はresult単独を禁止し、last targetならtarget result / plan Completed / summary / latest pointerまで同一guarded closureへ含める。全target terminal後のnon-success / preexisting recoveryはplan stateでなくsummary ExitedWithRecoveryに表す |
| Pending ReconciliationRequiredのreasonをpublic safe failureへ写す一方、canonical recordに保存先がない | failure projectionの型矛盾 | Pending recordへReconciliationRequired専用bounded `reconciliation_reason: Option<SafeOperationFailure>`を置き、他stateではNone、Failedはstate内failureだけをauthorityにする。public lifecycleと別のsingle safe_failureへ一意写像し、二重保持 / frontend mergeを禁止する |
| shutdown targetをCompletedへ畳むだけでは4値resultとfailureを失う | target read modelの情報欠落 | `ShutdownTargetView`へ`terminal_result: Option<ObligationResult>`、safe observation、single safe failureを持たせる。Completedは4値result必須、FailedTerminalだけfailureあり、derived CancelledBeforeActivation / Supersededはresult / failureなし、ReconciliationRequiredはresultなし＋保存済みreasonとする。frontendはstateだけからresult / failureを推測せずbackend projectionを表示する |
| `EffectReserved` commit後・adapter call前に別resolverがclaim / scope fenceを削除でき、既にterminalなeffectを遅延workerが開始し得る。またadapter handoffがI/O / lock待ちを行うとStopの10秒budget自体を消費する | reservationとI/O dispatch間のTOCTOU / unbounded handoff | 全external I/Oを`EffectDispatchGate(effect_id)`へ集約する。invocation直前にPending(EffectReserved)、terminal Absence、revision、claim generation / token / boot / lease、effect / intent hash、owner / association、dispatch fenceのowner revision / command・runtime・executor generation、scope fence、external recovery action attempt generationを再検査する。shutdownはActivated lineage / page hash / ordinal / target-authority digestとidentity / Present状態 / cutoffも検査する。gate内handoffはnon-async / non-blockingで、filesystem / socket / process I/O、provider response、Session / runtime-event lockその他のlock待ちを禁止する。validated immutable commandをcancellation-shielded owned driverへ移し、I/O開始権と一体のstarted handleをregistryへtry-registerするだけとする。登録不能ならI/O開始前にeffect 0件で失敗し、result waitはgate解放後のabsolute deadline残予算だけで行う。terminal / resolution / reclaim / invalidationも同じgate＋registryでfirst-winnerを決め、未検査queueへの遅延first dispatch、Stop budgetを消費するhandoff hang、全pathのstale dispatchをeffect 0件で拒否する |
| 保存済みsafe observation文字列だけではUseObservedResult / CancelIfSafeのauthoritative proof、owner、digest、retentionを再検証できない | proof authority不在 | provider / runtime / workflow adaptorだけが認証済みresponseまたはcanonical eventから`AuthoritativeEffectProofRefV1`を作る。effect ID、external correlation hash、safe observation hash、private blob schema / 1 byte..1 MiB / content hash / owner accessへ束縛し、client supplied proofを禁止する。kindはEffectStarted / Succeeded / Terminal / AuthoritativeNotFound / ConfirmedNoEffect / Ambiguousに閉じ、proof ref＋pending revision＋safe observationを同closureで確定する。public viewはsafe digestだけ、Terminalはproof refを持たずsafe observation digestだけを残し、private blobは全参照消滅後だけGCする |
| ObligationKindとpayload / effect / owner / optional turn・operation fieldの組合せがclosedでなく、wrong-kind intentやSessionClose effect NoneのPendingをimport / claimできる | common exhaustive validator不在 | 9 kindのclosed matrixを固定しbuilder / replay / import / claim / action resolverで共用する。Turn / QueueはSession・association None・turn / operation Some・5 lifecycle、TerminalCommitはSession・association None・turn Some / operation None・Stop / ProviderInterrupt・3 lifecycle、PermissionDeliveryはSession・両correlation None・PermissionResponse・5 lifecycle、ProviderEstablishはSession・turn None / operation Some・ProviderCreate / Resume・5 lifecycle、BackendRecoveryはSession / Orphan・両correlation None・ProviderCreate / Resume・5 lifecycle、SessionCloseはSession・両correlation Noneでeffect / scope SomeならRuntimeClose＋5 lifecycle・both Noneなら直接Terminal、WorkflowShutdownはWorkflow・両correlation None・5 lifecycle、RecoveryPublicationはSession / Orphan・association / correlation / external effect / dispatch fence / provider proofなし・BackendRecovery dependency exact 1・3 lifecycleでEffectReserved禁止とする。unexpected Some / Noneはreject / quarantineする |
| Application shutdown plan全体のactionが個別recovery用5値`OperationAction`を流用すると、Retry Quitを表現できず安全条件も失う | plan actionとtarget actionのdomain混同 | plan projection専用closed enum `ApplicationShutdownAction::RetryQuit`へ分離する。返却条件はsame-boot pre-activation Failed、external effect 0件、durable Failed fence、global admission Open、store Healthy、unresolved shutdown scope fence 0を同snapshotで満たす場合だけ。Preparing / Prepared / Activated / Quiescing / Completed / Cancelled / ReconciliationRequired、activation unknown、process-only failure、Stalled / unhealthy、admission Closed、scope fence残存、fresh bootは空とし、明示retryだけでguardを迂回しない。個別obligation / targetだけが5値OperationActionとresolve commandsを使う |
| shutdown target key / obligation IDのbyte encodingが未固定だとprepare、restart、F3 import、Tauri / WebSocketで同じtargetが別identityになる | shutdown identity preimage / encoding不在 | target keyは`LP("application-shutdown-target/v1") + LP(target tag) + LP(stable owner id)`、obligation IDは`LP("application-shutdown-obligation/v1") + LP(plan_id) + epoch(u64 BE) + LP(target_key)`をそれぞれSHA-256しbase64url no-pad化する。LPはu32 BE byte length＋UTF-8 raw bytes。enumeration / ordinal / cursorはencoded target keyのUTF-8 byte順へ統一し、raw preimageをkeyにしない |
| final parts、assistant message、terminal event、Session projection、queue pauseが別writeで、process内compensationしかない | L-P2 / L-P6 | schema-versioned Phase 0 redo manifestとterminal direct identity。terminal gateは`(session, turn)` winnerを一つにし、StartedはTurnExecution、queued由来はreserved turn IDのQueueExecution umbrellaをexactly one取得する。Stopがあれば単一TerminalCommitも取得し、terminal / final parts / assistant / Session・permission・queue / umbrella result / Stop result / claim・index削除 / occupancy releaseを同closureで確定する。Stop winnerのInterrupted(User/Timeout)は両result Succeeded、normal/Fatal/SessionClosed/競合winnerはumbrella Succeeded＋TerminalCommit Supersededとする。commit pointはmanifest / COW / root hash検証とroot CAS後のroot file / parent / required ancestor sync成功であり、visible / reachableだけではCommittedにしない。OutcomeUnknownはsame attemptへjoinして再検証・再syncし、不能ならStillUnknown / Stalled、BeforeCommitはrollback ackまたは旧writer不在を証明したrestart後fresh absenceだけに限定する |
| terminal uniquenessがlast terminal比較 / event全scanに依存し、`t2`終端後のlate `t1`がstateを書き換え得る | turn identity / bounded CAS不在 | `(session_id, turn_id)` direct recordとcurrent-turn revision / epoch fence |
| watchdogがruntime event lock / provider awaitへ従属し、Stopから10秒の保証を超える。また同時Stopを無制限にAcceptedするとsingle writerで全件deadlineを保証できない | I5 / L-D2 | request ingressでunresolved occupancyを先取りしたsame-boot monotonic `tokio::time::Instant`をlogical T0とし、StopAcceptance commit成功時だけ対応するUTC `accepted_at`をpublic監査fact、別の`acceptance_committed_at`をdurability監査時刻として保存する。same-target join、distinct最大32件 / serialized 4 MiBのoccupancy＋deadline scheduleを使う。StopAcceptance / terminal critical manifest各125 ms、32 targetsの2 commitで8s＋余白。`T0 + 1.5s ... 9.25s`のforce slot、9.5秒commit、残り0.5秒query / emit、10秒public deadlineとし、same bootの判定にwall clockを使わずresponse遅延でdeadlineを後ろへ動かさない。restartではpersisted UTCを監査 / foreign-boot期限分類にだけ使い、期限切れまたは残時間を証明できないrecordをR-009へ送り、accepted_at再採番・deadline延長・provider interrupt再実行をしない。10秒でReconciliationRequiredでもterminal resultまではoccupancyを保持し、startupも復元する。33件目 / schedule不能はpre-provider `StopCapacityExceeded` |
| Stop terminal保存失敗後に回復情報を作るため、同じstorage障害または直後crashでobligationも失う | durable-first ordering違反 | Stop Accepted前にtarget turn、runtime epoch、interrupt intent、queue pause、absolute deadline / fenceを持つ`TerminalCommit(EffectReserved)`とclaimをStopAcceptance manifestで同時確定し、Accepted後の別reserve commitを置かない |
| recovery公開snapshotがmemory-onlyでstartup discoveryは個別load / event履歴に依存する | I2 / bounded recovery authority不在 | 全obligation kindのpending-only inventory、claim generation / token / worker owner / 30秒lease。inventoryは各pendingを一度だけ置く6-partition canonical primary / count、actual-owner secondary、shutdown-association secondaryの3 COW tree refを同一revisionで切り替える。All / Partition、Owner、ShutdownPlan queryはそれぞれprimary、owner secondary、association secondaryからprimaryへbounded lookupし、countはprimaryだけを使う。domain ownerは`Session / WorkflowExecution / OrphanRuntime`のclosed enumとし、quit targetも実ownerを保持したまま別のimmutable `ApplicationShutdownAssociation { plan_id, epoch, target_key }`で旧plan completion / queryをfenceする。association付きはowner / lifecycle変化後もterminalまでApplicationShutdown primaryへ固定し、association secondary keyを変更しない。runtime detachment、OrphanRuntime recovery obligation、UnownedRuntime primary / owner secondary / root envelope更新を同transactionで確定し、memory-only orphanを回収identityへしない。BackendRecovery completionから独立RecoveryPublication pendingへmanifest handoffし、message / payload / marker / terminal authorityをpublication obligationだけに置く。個別Session loadや全履歴scanをdiscoveryに使わない |
| storage failure feedbackのscope確認が同じ壊れたSessionMeta再読込に依存し、stale successが新しいfailureをclearし得る | P4 / feedback identity不在 | request session idによるscope、未解決failureだけのfeedback / attempt / revision identity、linked-success clear。`GetSessionOperationFeedbackRequest { session_id, cursor, limit: 1..=32 }`とprocess全体512件のidentity collectionを使い、未解決entryをevict / coalesceせずclose / archiveでも自動削除しない。Loadを含むfeedback対象domain operationは開始前slot予約・成功時解放とし、予約不能時は対象へ作用前にFeedbackCapacityExceeded、mutation effect 0件。dismiss / retryは`feedback_id + expected_revision`とretry時のRust-owned action IDを使うcapacity-exempt control planeとして両surfaceへ公開する。成功とretry再失敗は更新後snapshot付き`Applied`、unknown / stale / action不正はeffect 0件の`Rejected(NotFound / RevisionConflict / ActionUnavailable)`、control storage failureは新feedbackを作らない`Failed`とする |
| F1 goldenがtest reducerだけを通りproduction runtime event / terminal / projectorのdriftを検出しない | F1 safety netのproduction-path gap | Claude / Codex F1b production golden |
| close、archive、backend switch、view close、application quitが同じ「close」として混在し、正本I1もbackend switchの意味論が矛盾する | I1 / typed surface authority不在 | close / quit decision table。view closeはeffect 0。normal close / open archiveの初期closureはactiveならterminal＋queue pause＋projection、Idleならterminalなしでqueue pause＋projectionを確定し、runtime scopeありだけscope / effect both SomeのSessionClose(Pending)を同時予約、EffectReserved後にcloseして観測closureでTerminal化する。scopeなしは初期closureでboth Noneのcompact Terminal(Succeeded)を直接確定しpending / effect 0件とする。closed archiveはprojectionのみ。unknownは確定済みstateを保持してreconciliation。backend switchはIdle / pendingなしだけ受理し、old scopeありの初期D1 closureはold guard＋queue pause＋SessionClose(Pending)、観測closureでTerminal化した後だけnew effective、scopeなしは初期closureでcompact Terminal(Succeeded)＋new effectiveを確定する。既存item保持 / 不整合NeedsResolution。active / Idle normal close、active / Idle open archive、closed archive、Idle backend switchはingress T0から10秒以内に完了またはsame identityの結果未確認へ確定し、runtime close unknownではClosed / Archived＋paused queue、activeだけSessionClosed、switchはold effective、closed archiveはqueue不変を保つ。late resultで別effect / terminal / reopen / new backend startを作らない |
| Cmd-Q、menu、Dock、tray、native exitがsingle-flight coordinatorへ集約されず、shutdownにdeadline・workflow obligation・retry epochがない | I17 / bounded shutdown不在 | Session / Workflow / Application scopeを分け、quit開始時にopenなactive / Idle Sessionと進行中Workflowだけをeffect targetとしてprepareする。same-boot current nonterminal flightは異intentでもsame resultへjoinする。process flight不在でsame-bootまたはprevious-bootのnonterminal latestまたはunresolved shutdown scope fenceが残るrequestはexact `PreviousShutdownReconciliationRequired`でnew plan identity / root / page / effect 0件のBeforeCommit rejectionにする。same-bootまたはprevious-bootのprior latestがeligible terminalでnew-flight admission / store / explicit retry guardを満たしたrequestだけがminimal durable `Preparing` rootと`LatestShutdownAttemptRefV1`を同closureで確定し、以後の全transitionも同pointerをCASする。global activationは別の`LatestActivatedShutdownPlanRefV1`をrootと同closureでCASし、commit確認後のcurrent coordinatorだけがper-target EffectReservedを明示shutdown commandのgateにする。cross-plan duplicate gateはdomain-separated `(ObligationOwner, exact OwnedShutdownScopeRef)` keyのdirect `UnresolvedShutdownScopeFenceV1` / COW indexをauthorityとする。reservation同commitでAbsent→insert、Pending EffectReserved / ReconciliationRequired / Failedの間保持、Terminal同commitでdeleteする。root-initはscope-fence root count 0をguardし、既存entryがあればnew plan / root / page / effect 0件の`PreviousShutdownReconciliationRequired`、root-init後のcandidate point lookupは新たなfenceだけをpre-activation abortへ送り、Phase 0はhandoff / reuseせずproven new generationの別keyだけを許す。Phase 0 fence rootはF3同key UNIQUE authorityへparity importする。関連provider runtime / childはowner targetのsubordinate effectとし別targetへ数えない。通常時に3-tree pending recovery inventoryを保守し、composable Merkle nodeでmutation / range hashをO(path)に限定してO(N) ordered-leaf再hashを禁止する。shutdownはcanonical primaryのClosedSession / ArchivedSession / UnownedRuntime ordered 3 rangeについてinventory revision、root envelope / range hash、countだけをrecord列挙なしでsnapshotし、secondaryや他の3 primary partitionをcountへ混ぜない。owner detachment / orphan obligation / primary / secondary / root更新は同transactionとし、memory-only orphanはsummaryへ含めない。preexisting pendingはtarget / 4096 countへ含めずそのrefでexit / restart監督し、quit時の200件page複製や新effectを行わない。13秒durability cutoffは`T0`基準のabsolute slotで閉じる。`[0, 0.25s]`はinitial `ShutdownRootInitialized` 1 closure、`[0.25s, 2.0s]`は残りadmission settle 1.75秒、`[2.0s, 3.25s]`はtarget / recovery snapshot enumeration / encode 1.25秒、`[3.25s, 11.75s]`は最大32 page＋Prepared root＋activationの34 closuresを各250 msで8.5秒、`[11.75s, 12.5s]`はOutcomeUnknown lookup / scheduling 0.75秒、`[12.5s, 12.75s]`はactivation未成立時だけdurable Failed / Cancelled finality 1 closure、`[12.75s, 13.0s]`は0.25秒marginである。途中failureは残りpage slotを捨ててfinality slotへ進み、initial initが`BeforeCommit`なら未受理のままfinalityを作らない。各Session deadlineは`min(executor開始 + 10s, cutoff)`、global exitは15秒とする。prepare/root failureまたはactivationのBeforeCommit＋rollback ack＋latest-attempt fence確認時だけprocess exitなし・effect 0件でabort / admission reopenする。activation OutcomeUnknownはabortせずcommand 0件のまま15秒でactivation-possible recovery exitし、fresh bootが同identityをresolveする。activation後もterminal / completion / EffectReserved未到達targetをprepared identityで残し、pipe close / job object / parent-deathの暗黙作用を0と推測せず`ExitCoupledOutcomeUnknown`へ送る。current projectionはTauri `get_application_shutdown` / WebSocket `GetApplicationShutdown`からclosed `CurrentApplicationShutdownResult`として返す。hash-validなcomplete rootがexactly oneでplan ID / epoch / exit intentを一意にanchorでき、pointer等の冗長semantic driftだけが矛盾する場合に限ってsafe `ShutdownAuthorityMismatch`を持つ同identityのReconciliationRequiredへ写す。storage / read / decode / envelope / self-hash / pointer-to-root hash failure、required record欠損、state composite / activation lineage integrity failure、identity unanchorable / ambiguousは`GetApplicationShutdownApplicationError::Internal { correlation_id }`で返し、`None`、`OutcomeUnknown`、ReconciliationRequiredへ包まない。current pending一覧は`ListPendingRecovery`、plan snapshotはexact `GetPendingRecoverySnapshot`へ分ける。shutdown readbackはscope-fence rootと2 pointerを同一leaseへpinし最大32 page / 4096 targetを2秒以内にexact foldする。ExitCoupled candidateはinitial Activatedならlatest-attempt root hash、post-activation descendantならimmutable `activation_ancestor_sha256`をlatest-activated pointer hashへ照合し、previous-boot Prepared activation-possible caseと合わせて最大1件だけ選ぶ。`terminal_count`はcompact 4値resultを数え、`completed_count`はtarget単位で4 terminal値またはderived Superseded / CancelledBeforeActivationの和集合を重複なく含め、partial / mixed revisionを返さない。pre-activation Failed / Cancelledのterminal closureはeffect 0件の`AbortedBeforeActivation(details=Available)`としてfull page / target authority / snapshot refを保持し、同closureでarchive insert、detail detach、GCを行わない。full detailsを保持できるのはlatest plan（current Preparing / Preparedを含む）＋at most one retiring planだけで、new flightのroot-initはeligible terminal prior latestがAvailableかつretiring Noneなら旧latestをretiringへreserveしnew Preparing root / latest-attempt pointerと同一closureで確定する。retiringが既にSomeならexact `PreviousShutdownCompactionPending`でnew plan identity / root / page / effect 0件のBeforeCommit rejection、priorがCompactedならarchive parityを検証してretiringを変更せずnew root-initへ進む。root-init / Prepared / activation commit path外のguarded background compactorは`ArchiveSwitch → DetailDetachChunk → FinalizeDetach`を進める。ArchiveSwitchはCompacted archiveとPhase 0 cleanup cursorを持つcompact rootを同closureへ置き、detail delete 0のままqueryをarchive-onlyへ切り替える。Phase 0 detail / page cleanupはbounded ordered batch overlayとroot ordinal / prefix / residual suffix / pending transactionを先にcommitし、post-commit materializerのpartial file deleteを同batchから再開する。全page absence後の0-page dedicated closureだけpointerをclearする。future F3 SQLはInventoryDetachChunkのrow / item stateとlast atomic page batch / zero-page clearを使う。logical commit、materialization、final clear前後のcrashも同authorityから再開し、ArchiveSwitch後のqueryをAvailableへ戻さない |
| concurrentなquit surfaceが異なるexit code / exit・restart modeを要求したとき、後着requestがcoordinatorの終了intentを書き換えると最終permit / summary / UIがfirst accepted ingressと食い違う | exit intentとrequest identityのfirst-winner authority不在 | adaptorで全surfaceをcanonical `ShutdownExitIntent { mode: Exit / Restart, code: i32 }`へ正規化する。同じprincipal / request ID / same intentはsame result、same request ID / different intentはresult variantやSafeOperationFailureではなくWebSocketの`AgentSessionWsErrorV1::PayloadConflict { identity: ApplicationQuit { request_id } }`またはTauriの`RequestApplicationQuitApplicationError::PayloadConflict { request_id }`＋effect 0件とする。別request IDでcurrent flightへ到着した後続quitはfirst accepted intentへjoinし、flight、plan、summary、projection、one-shot permitを書き換えない。pre-activation effect 0 abort後のnew flightだけnew intentを採用する。`ShutdownExitIntentV1`はadaptor DTOに限定する |
| 暫定file-store bridgeと後続SQLiteを分けるとtransaction / persistence DTOを二度実装し、authority移行の穴を増やす | L-P6 / F2・F3を分離した旧実装順 | F2 #1384とF3 #1385を#1499へ統合し、Primary Designのdomain型 → SQLite transaction → lifecycle closure → Legacy直移行を同じreleaseで実装する。managed backup / restore public lifecycleだけはnon-goalとする |

本節が参照する#1499の公開query境界は次で固定する。通常sendは既存`send_agent_message`一つだけであり、`AgentSendOperationView`は`Accepted { receipt, status, obligations, available_actions } / OutcomeUnknown { operation_id }`のexact 2 variantである。Stopの`StopOperationView`は`Accepted { receipt } / Terminal { receipt, resolution, result } / ReconciliationRequired { receipt, failure, available_actions } / OutcomeUnknown { operation_id }`のexact 4 variantである。どちらも未知identityは`NotFound`、same identity / different payload・targetは各commandのtyped `PayloadConflict` application errorであり、viewや`SafeOperationFailure`へvariantを追加しない。current shutdownは`CurrentApplicationShutdownResult::Current(Option<ApplicationShutdownProjection>) / OutcomeUnknown { failure }`のexact 2 variantである。hash-validなcomplete rootがexactly oneでplan ID / epoch / exit intentを一意にanchorでき、pointer等の冗長semantic driftだけが矛盾する場合に限って`ShutdownAuthorityMismatch`付きReconciliationRequiredへ写す。storage / read / decode / envelope / self-hash / pointer-to-root hash failure、required record欠損、state composite / activation lineage integrity failure、identity unanchorable / ambiguousは別の`GetApplicationShutdownApplicationError::Internal { correlation_id }`で返して`None`、`OutcomeUnknown`、ReconciliationRequiredへ包まない。known quit operationのreadbackはOptionを持たない`CurrentApplicationQuitOperationResult`とclosed `ApplicationQuitProjection::Shutdown | Bootstrap`を使い、bootstrap-safe flightをnormal planへ合成しない。

view closeはviewだけを閉じ、Session terminal、flush、permission、queue、runtime、workflow、shutdown progressを一切変更しない。Phase 0の公開runtime surfaceにはresource-isolated input、operation / resource privacy purge、managed backup / restore、app-data reset、complete privacy authority reset、export / importを追加しない。

#1499のbounded recovery / shutdown解消契約では、pending discovery用3-tree envelopeとは別にPending / Terminal全てをID lookupできるimmutable obligation-state-by-ID COW rootと、cross-plan duplicateを防ぐunresolved shutdown scope-fence COW rootを持つ。obligation transitionはdirect record、state root、pending中だけ存在するprimary / secondary / root envelope、該当shutdown scope fenceを同じmanifestで更新し、transaction-inventory commit前にlogical rootsを進めない。startup / common recovery queryは`Phase0ReadSnapshotRef`でtransaction inventory、pending envelope、state root、latest-activated pointerを同じrevisionへpinする。shutdown exact fold / action / compactorは同じcommit間隙でscope-fence rootとlatest-attempt pointerを追加した`Phase0ShutdownReadSnapshotRefV1`を使い、baseだけからcurrent candidateを合成しない。これによりterminalをpending indexから消した後も4096 target foldがwriter再開後の別revisionを混ぜず、event / Session history scanなしに、compact 4値resultを`terminal_count`へ数え、4値resultまたはderived Superseded / CancelledBeforeActivationのtarget単位和集合を重複なく`completed_count`へ数えられる。

#1499のpublic shutdown phaseは`Preparing | Prepared | Activated | Quiescing | Completed | Failed | Cancelled | ReconciliationRequired`のclosed 8値である。new flightにeligibleなprior latestは`Completed`、またはdurable `Failed | Cancelled` terminal fenceと、同じplan / epoch、counts、safe failure、exit intentを持つeffect 0件の`AbortedBeforeActivation` terminal rootが揃う場合だけである。`details=Available`ならfull refs / snapshotを保持したpriorをnew root-initでretiringへreserveし、`details=Compacted`なら`ShutdownPlanCompactArchiveV1`とのparityを検証する。admission Open、store Healthy、unresolved shutdown scope fence 0を全caseで必須とし、process-only Failed、Stalled / unhealthy、admission Closed、scope fence残存はexplicit retryでもnew plan / root / page / effect 0件である。`RetryQuit`もsame-boot effect 0 Failedとこの全predicateを満たす場合だけ提示する。

shutdown snapshot capacityはtarget authority 1〜65,536 bytes、1 page 128 target refsかつencoded 1 MiB以下、root 32 pages / 4096 targets以下である。65,537 bytes、1 MiB＋1、129 refs、33 root pages、4097 targetsはBeforeCommit capacity failureとしてnew object / effect 0件とし、上限ちょうどを受理する。

wire contractはRust u64 semantic fieldのepoch / revision / sequence / ordinal / count / offset等だけをcanonical decimal stringとし、bounded transport controlの`limit: u16` / `max_bytes: u32`はJSON nonnegative integer、exit codeの`i32`はJSON signed integerにする。wrong representation、fraction、型 / route範囲外は`InvalidRequest`＋effect 0件である。通常sendのcaller operation IDはWebSocket outer request IDとは別で、Tauri / WebSocketのcurrent-installation principal scopeへ、現行6 target field＋content / images / mentions / editor context / active-turn policyのsame exact payloadと共に束縛する。Accepted receiptはopaque `input_ref`を必須とする。

Accepted receiptの`StartedTurn | Queued` dispositionはprovider establish待ちでも不変で、execution statusだけを`AwaitingProviderStart`とし、dependency success前にProviderStartReserved / Runningへ進めない。Accepted後のReconciliationRequired / Failedもreceiptを維持し、受理前failureへ戻さない。readbackの`ConfirmedNoEffect`から`CancelledBeforeEffect`へ進めるのはcancel policyが許可するtargetだけで、non-cancellable targetとQueueExecutionには`CancelIfSafe`を提示せずdirect requestも`ActionUnavailable`＋effect 0件とする。
