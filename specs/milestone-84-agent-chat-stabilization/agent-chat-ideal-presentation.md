...
- **P3 (無言遷移の禁止)**: ユーザーに観測可能な状態変化（turn 終了・失敗・中断・permission 失効・queue 変化）は、必ず画面上の変化を伴う。「スピナーが消えただけ」「reload したら突然エラーが現れる」（FE-2）を許さない。
...
- **P6 (監査可能性)**: 後からセッションを開いた読者が「何が実行され、なぜ止まり、何が拒否され、いくら使ったか」を transcript から読み取れる（RG-4 / FE-7 / CL-1 effective / RG-9）。
...
| turn_phase / stall | S1 末尾 ＋ S8 | S2 | Streaming: spinner。stall 診断 Notice 到着で「応答が観測できません（経過時間）」表示に切替（I11）。Interrupting: 「停止中（最大 10 秒）」 |
| queue | S5 | — | chips: Queued / Paused / Failed / Cancelled を可視化（OB-3/OB-5/OB-6）。Paused は再開ボタン、Failed は再試行・取り消し |
...
| UnsupportedMessage / OversizeDropped | S1 inline（低強調） | 「未対応の応答を N 件受信」など。透明性を優先しつつ目立たせない（V-P1 の可視化） |
...
- **Question**: question ごとに描画し、`is_secret` はマスク入力、`is_other_allowed` は自由記述欄、multi_select は複数選択 UI（CX-1 の語彙前提）。
